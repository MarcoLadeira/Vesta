# OPai Local Execution: `opai ask` (real savings, not estimates)

For a long time OPai *planned* the cheap route but never ran it - the savings
...
1. **Classify** the task with OPai's model-intelligence taxonomy.
...
3. **Run locally** on a compact, already-redacted evidence prompt (OPai's
...
- **Loopback/private only by default.** OPai only talks to a local model on a
  loopback/private endpoint (reuses the #19 endpoint classifier). A public
  `LOCAL_MODEL_URL` is refused - OPai never silently ships your prompt to a host.
- **Cloud is never auto-called.** If the recommended tier is cloud/paid, `opai
