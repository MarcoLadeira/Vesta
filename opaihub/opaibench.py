...
def _git_runtime(root: Path) -> tuple[list[str], dict[str, str]]:
    ...
    sandbox = root.parent / ".opaibench-git"
    hooks = sandbox / "hooks"
        ...
        (
            ...
            [
                *git,
                ...
                "user.name=OPaiBench",
                "-c",
                "user.email=opaibench@invalid.local",
                "commit",
                "--quiet",
                "-m",
                "fixture baseline",
...
def _run_parity_task(
    ...
    *,
    ...
    started = time.perf_counter()
    with tempfile.TemporaryDirectory(prefix=f"opaibench-{task['id']}-") as tmp:
        root = Path(tmp) / "repo"
        ...
        unexpected = sorted(set(changed) - allowed - {".opai-app.json"})
        edits_correct = (
...
def run_parity_benchmark(
    ...
    *,
    ...
    if write:
        directory = opaibench_dir(Path(project_root).expanduser().resolve()) / "parity"
