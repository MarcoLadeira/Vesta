# OPai Auth and Message Reliability Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make provider authentication, terminal message state, activity, branding, recovery, and connection diagnostics truthful and safe across the OPai desktop chat.

**Architecture:** Normalize every provider process outcome at the Python CLI boundary into one safe error/connection contract. Preserve that contract through the pipeline and Qt bridge, then let a pure JavaScript message state machine drive OPai-first rendering and reject invalid terminal transitions.
...
**Files:**
- Create: `opai/provider_contract.py`
...
Expected: import failure for `opai.provider_contract`.
...
Run: `git add opai/provider_contract.py tests/test_provider_contract.py && git commit -m "feat: normalize provider authentication errors"`

### Task 2: Truthful account runner outcomes

**Files:**
- Modify: `opaihub/accounts.py:54-68,410-545`
...
def test_nonzero_auth_exit_is_failure(self):
    ...
    with mock.patch("opaihub.accounts._popen", return_value=proc):
...
Run: `git add opaihub/accounts.py tests/test_streaming.py tests/test_copilot_connector.py && git commit -m "fix: preserve provider process failures"`

### Task 3: Connection state and safe health checks

**Files:**
- Modify: `opaihub/accounts.py`
- Modify: `opai/app_state.py:512-572`
...
Run: `git add opaihub/accounts.py opai/app_state.py tests/test_provider_connections.py && git commit -m "feat: expose honest provider connection states"`

### Task 4: Pipeline terminal truth and OPai activity vocabulary

**Files:**
- Modify: `opai/activity.py:21-41,158-312`
- Modify: `opai/app_state.py:633-776`
- Modify: `opaihub/gui_pipeline.py:136-281`
...
Emit `provider_checking`, `provider_authenticated`, `provider_auth_failed`, `failed`, `cancelled`, `retrying`, and `completed`. Put raw provider/model IDs in metadata. User-facing titles say OPai.
...
Run: `git add opai/activity.py opai/app_state.py opaihub/gui_pipeline.py tests/test_activity.py tests/test_message_contract.py tests/test_streaming.py && git commit -m "fix: make request terminal states truthful"`

### Task 5: Browser message state and branding

**Files:**
- Create: `opai/assets/web/message-state.js`
- Create: `opai/assets/web/__tests__/message-state.test.js`
- Modify: `opai/assets/web/index.html`
- Modify: `opai/assets/web/app.js:28-35,245-487`
- Modify: `opai/assets/web/activity.js`
...
Run: `npm run test:unit -- opai/assets/web/__tests__/message-state.test.js`

Expected: module/script missing.

- [ ] **Step 3: Implement reducer and OPai-first rendering**

The assistant role, pending label, completed response, and primary selector use OPai names. `advancedLabel`, provider, and model remain in Inspector/details. Error cards consume `result.error` and render each sanitized message once.
...
Run: `git add opai/assets/web/message-state.js opai/assets/web/__tests__/message-state.test.js opai/assets/web/index.html opai/assets/web/app.js opai/assets/web/activity.js && git commit -m "feat: add truthful OPai message states"`

### Task 6: Connections bridge and settings UX

**Files:**
- Modify: `opai/gui_web.py:174-470`
- Modify: `opai/assets/web/app.js:553-584`
...
Run: `git add opai/gui_web.py opai/assets/web/app.js opai/assets/web/styles.css opai/assets/web/__tests__/e2e/mock-bridge.js tests/test_gui_web.py && git commit -m "feat: add provider connections manager"`

### Task 7: Playwright auth/message flows

**Files:**
- Create: `opai/assets/web/__tests__/e2e/auth-message.spec.js`
- Modify: `opai/assets/web/__tests__/e2e/mock-bridge.js`

- [ ] **Step 1: Add invalid-auth, missing-auth, success, retry, branding, and activity assertions**

```javascript
test("invalid auth is one failed OPai card without completed activity", async ({ page }) => {
...
Run: `npx playwright test opai/assets/web/__tests__/e2e/auth-message.spec.js`
...
Run: `git add opai/assets/web/__tests__/e2e/auth-message.spec.js opai/assets/web/__tests__/e2e/mock-bridge.js opai/assets/web/app.js opai/assets/web/styles.css && git commit -m "test: cover auth and message reliability flows"`
...
- Create: `docs/OPAI_ERROR_SYSTEM.md`
...
Run: `npm audit --json`, targeted secret-pattern `rg`, `python -m pip check`, and `opai scan`. Save no secret values; record only file/line evidence and advisory summaries.
...
Run: `python -m compileall -q opai opaihub opcoding`, `python -m pip check`, `npm audit --omit=dev`, and `git diff --check main...HEAD`.
