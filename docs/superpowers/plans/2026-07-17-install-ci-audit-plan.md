...
- **Create:** `scripts/install_vesta.py` — testable installer logic
- **Create:** `tests/test_install_vesta.py` — unit tests for installer logic
...
**Files:**
- Create: `scripts/install_vesta.py`
- Test: `tests/test_install_vesta.py`

- [ ] **Step 1: Write the failing tests**

```python
# tests/test_install_vesta.py
...
import scripts.install_vesta as install_vesta
    ...
    def test_install_with_pipx_runs_pipx_install(self):
        with mock.patch("shutil.which", return_value="/usr/bin/pipx"):
            with mock.patch("subprocess.run", return_value=FakeCompleted()) as run:
                self.assertTrue(install_vesta.install_with_pipx())
                run.assert_called_once()
                self.assertIn("pipx", run.call_args[0][0])
                self.assertIn("install", run.call_args[0][0])
                self.assertIn("vesta", run.call_args[0][0])
...
class VerifyInstallTests(unittest.TestCase):
    def test_verify_install_runs_vesta_version_and_doctor(self):
        with mock.patch("subprocess.run", return_value=FakeCompleted()) as run:
            self.assertTrue(install_vesta.verify_install())
    ...
    def test_verify_install_returns_false_on_failure(self):
        with mock.patch("subprocess.run", return_value=FakeCompleted(returncode=1)):
            self.assertFalse(install_vesta.verify_install())


class MainTests(unittest.TestCase):
    def test_main_check_mode_returns_0_when_verify_passes(self):
        with mock.patch.object(install_vesta, "check_python_version", return_value=True):
            with mock.patch.object(install_vesta, "verify_install", return_value=True):
                result = install_vesta.main(["--check"])
                self.assertEqual(result, 0)

    def test_main_check_mode_returns_3_when_verify_fails(self):
        with mock.patch.object(install_vesta, "check_python_version", return_value=True):
            with mock.patch.object(install_vesta, "verify_install", return_value=False):
                result = install_vesta.main(["--check"])
                self.assertEqual(result, 3)

    def test_main_source_only_skips_pypi(self):
        with mock.patch.object(install_vesta, "check_python_version", return_value=True):
            with mock.patch.object(install_vesta, "install_with_pipx") as pipx:
                with mock.patch.object(install_vesta, "install_from_source", return_value=True):
                    with mock.patch.object(install_vesta, "verify_install", return_value=True):
                        result = install_vesta.main(["--source-only"])
                        self.assertEqual(result, 0)
                        pipx.assert_not_called()

    def test_main_returns_2_when_all_install_methods_fail(self):
        with mock.patch.object(install_vesta, "check_python_version", return_value=True):
            with mock.patch.object(install_vesta, "install_with_pipx", return_value=False):
                with mock.patch.object(install_vesta, "install_with_uv", return_value=False):
                    with mock.patch.object(install_vesta, "install_from_source", return_value=False):
                        result = install_vesta.main([])
...
Run: `python -m pytest tests/test_install_vesta.py -v`
Expected: FAIL with `ModuleNotFoundError: No module named 'scripts.install_vesta'`

- [ ] **Step 3: Write minimal implementation**

```python
# scripts/install_vesta.py
from __future__ import annotations
...
def install_with_pipx(package: str = "vesta") -> bool:
...
def install_with_uv(package: str = "vesta") -> bool:
    uv = shutil.which("uv")
    if not uv:
        return False
...
def verify_install() -> bool:
    for cmd in [
        [sys.executable, "-m", "vesta", "--version"],
        [sys.executable, "-m", "vesta", "doctor"],
    ]:
        result = run(cmd, capture_output=True)
        if result.returncode != 0:
            return False
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Production Vesta installer")
...
```bash
git add scripts/install_vesta.py tests/test_install_vesta.py
...
class InstallPs1Tests(unittest.TestCase):
    def test_install_ps1_calls_python_installer_script(self):
        ...
        self.assertIn("scripts/install_vesta.py", content)
...
Expected: FAIL — current `install.ps1` does not reference `scripts/install_vesta.py`
...
Expected: FAIL — current `install.sh` does not reference `scripts/install_vesta.py`
...
PYTHON="${VESTA_PYTHON:-python}"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
SCRIPT="$SCRIPT_DIR/scripts/install_vesta.py"
...
- [ ] Run `python -m pytest tests/test_install_vesta.py tests/test_install_ps1.py tests/test_install_sh.py tests/test_ci_install_smoke.py tests/test_no_paid_terms.py -v`
...
**Type consistency:** `scripts/install_vesta.py` exports `check_python_version`, `install_with_pipx`, `install_with_uv`, `install_from_source`, `verify_install`, `main` — used consistently across tests and shell wrappers.
