# Vesta 0.1.0 Pre-Alpha Pre-Repo Hardening Plan

Date: 2026-05-02

Goal: make Vesta safe to publish as a one-command installable, local-first AI coding hub that materially improves coding speed and quality while minimizing paid model usage.
...
Vesta should feel like a quiet supercharger for any AI coding session:

1. The user installs it with one simple command.
2. Vesta appears automatically in supported CLI/AI coding clients.
...
- Vesta version: `0.1.0 pre-alpha`.
- Main packages: `vesta`, `vestahub`, `opcoding`.
...
- Vesta global integration has been installed locally for Codex/Claude/Copilot-style shell wrappers.

Current capabilities found:

- CLI entry points: `vesta`, `op`, `op-hub`.
...
- Vesta terminal UX: blue `Using Vesta` badge, mascot asset, ANSI/ASCII/kitty/iTerm rendering paths, animated compact welcome.
- Global AI-client discovery files: Vesta Codex skill, Claude memory block, shell wrapper scripts.
...
```powershell
python -m compileall opcoding vestahub vesta tests
python -m unittest discover -s tests
python -m vestahub validate
python -m vesta doctor
python -m vesta integrate status
python -m vesta hub analytics status
...
python -m pip wheel . --no-deps -w .vestahub\wheelhouse
python -m vesta --help
python -m vesta welcome --compact --animate --image ascii --frames 2 --delay 0
python -m vesta hub discover tools
python -m vesta hub sandbox check --command "git reset --hard"
python -m vesta hub schedule list
```
...
- `osv-scanner` returned "No package sources found"; Vesta needs clearer dependency source files or a better skip reason.
- A clean wheel install from outside the source tree fails `python -m vestahub validate` because `hub/` data is not packaged.
- `rg.exe` failed with "Access is denied" in this environment; Vesta should detect broken tool binaries instead of assuming installed means usable.
- The terminal animation prints raw ANSI when output is captured by non-interactive logs; Vesta needs TTY/CI detection.

## P0 Release Blockers

These must be fixed before creating a public repository or telling users to install Vesta.

### P0.1 Package The Hub Data Correctly

Problem:

- The built wheel includes `vesta/assets/vesta-mascot.png`.
- The built wheel does not include `hub/registry/*.yaml`, `hub/docs/*.md`, MCP examples, templates, prompts, or security configs.
- `vestahub.loader.hub_root()` falls back to `site-packages/hub`, but that directory does not exist in the wheel.
- Running `python -m vestahub validate` outside the source repo crashes with `FileNotFoundError`.
...
1. Update `vestahub.loader.hub_root()` resolution order:

```text
1. explicit --project/--hub-root argument
2. VESTA_HUB_ROOT environment variable
3. nearest ./hub in current project parents
4. packaged vestahub/data/hub via importlib.resources
...
```powershell
python -m vesta version
python -m vestahub validate
python -m vesta hub list-tools
python -m vesta doctor
```

Definition of done:

- `python -m pip wheel . --no-deps -w .vestahub\wheelhouse` succeeds.
- Wheel contains hub registry/docs files.
- Clean venv outside the repo can run `vestahub validate`.
- `vesta install --project <some-project>` can bootstrap from packaged defaults.
...
- `vestahub/health.py` uses shell string health checks with `shell=True`.
- `vesta/cli.py` uses `shell=True` on Windows launch.
...
- Vesta should be able to classify commands before execution, redact logs, and require confirmation for destructive operations.
...
```text
vestahub/command_policy.py
vestahub/command_runner.py
```
...
- `vesta hub sandbox check --command "git reset --hard"` and actual execution agree on risk classification.
...
- Dependency audit status appears in `vesta doctor`.
...
1. Treat `codingOP` as the Vesta repo root.
1. Do not push from the parent website repo.
1. Before initializing/pushing Vesta:
...
- A fresh repo rooted at `codingOP` contains only Vesta files.
- No parent website files appear in Vesta history.
...
- Vesta's brand promise is "simple CLI install, automatic AI coding upgrade".
...
```powershell
python -m pip wheel . --no-deps -w .vestahub\wheelhouse
python -m venv .vestahub\smoke-venv
.vestahub\smoke-venv\Scripts\python.exe -m pip install --no-index --find-links .vestahub\wheelhouse vesta==0.1.0
Push-Location $env:TEMP
.vestahub\smoke-venv\Scripts\python.exe -m vesta version
.vestahub\smoke-venv\Scripts\python.exe -m vestahub validate
.vestahub\smoke-venv\Scripts\python.exe -m vesta welcome --compact --no-color --image ascii
Pop-Location
```
...
4. Add `VESTA_ALLOW_CLOUD=1` or per-provider enable flags for any cloud path.
5. Make `vesta doctor` show missing keys without printing values.
...
These are the changes that make Vesta genuinely stronger than "a CLI with some YAML".

### P1.1 Build The Vesta Evidence Router

Purpose:

Before any AI reasoning, Vesta should gather cheap local evidence and choose the cheapest useful next step.

Add:

```text
vestahub/router.py
vestahub/evidence.py
vestahub/task_classifier.py
vestahub/cache_keys.py
...
```powershell
vesta route "fix failing auth test"
vesta explain-route "build a Stripe webhook endpoint"
vesta plan --budget cheap "add settings page"
```
...
Make Vesta useful across projects without dumping huge context into AI.

Add:

```text
.vesta/project-profile.yaml
.vesta/context/summary.md
.vesta/context/code-map.json
.vesta/context/test-map.json
.vesta/context/dependency-map.json
.vesta/context/commands.yaml
.vesta/cache/
```
...
```powershell
vesta project attach .
vesta context build
vesta context diff
vesta context show --changed
```

Definition of done:

- Vesta can attach any project and produce a reusable project context summary.
...
Most AI coding waste comes from broad context and broad test runs. Vesta should run the smallest useful checks first.

Add:

```text
vestahub/test_selector.py
vestahub/failure_analyzer.py
```
...
```powershell
vesta test --changed
vesta test --target tests/test_cli.py
vesta fix-tests
```
...
Vesta should help users commit safely without ever surprising them.
...
- `vesta git preflight` becomes the command to run before pushing.
...
The blue `Using Vesta` and mascot should feel polished, not noisy.
...
- CLI wrappers show `Using Vesta`.
- Codex skill instructs sessions to use Vesta.
- Claude memory block instructs sessions to use Vesta.
- Copilot instruction file can be copied into project instructions.

Reality check:

- Closed apps may not allow a true bottom-right status line. Vesta can provide wrappers, shell preamble, env vars, and instruction files; exact UI placement depends on each client.
...
```powershell
pipx install vesta
vesta install --global --shell-aliases
```
...
```powershell
python -m pip install --user vesta
python -m vesta install --global --shell-aliases
```
...
- `vesta doctor` explains what is active and what is missing.
...
- Vesta writes global files and shell profile blocks.
- Users need a clean exit path.

Add:

```powershell
vesta integrate uninstall
vesta integrate repair
...
Vesta should improve over time without spending more.
...
```powershell
vesta improve report
vesta improve suggest
vesta improve apply-safe
```
...
- Vesta can tell the user why it chose a cheap path and how much it saved.
- Repeated project work gets faster.

## P2 Advanced AI Coding Multiplier

These make Vesta feel closer to an "AI operating layer" while still staying local-first.
...
```powershell
vesta models detect
vesta models recommend --task bug_fix
vesta models enable ollama --model <name>
vesta models benchmark
```
...
```powershell
vesta index build
vesta search "auth middleware"
vesta context related path/to/file.py
```
...
```powershell
vesta workflow run feature_plan --task "add OAuth login"
vesta workflow run bug_fix --error-file .vesta/errors/latest.log
vesta workflow run release_prepare --confirm
```
...
```powershell
vesta mcp list
vesta mcp enable filesystem --project .
vesta mcp export codex
vesta mcp export claude
vesta mcp doctor
```
...
```powershell
vesta app create "todo app with auth and deployment"
vesta app ship
```
...
```powershell
vesta tool search playwright
vesta tool add ./tool.yaml
vesta tool enable ruff
vesta tool health --all
vesta tool doctor --category security
```
...
- Export client configs from Vesta, do not ask users to hand-edit risky JSON unless needed.
...
- "What Vesta can and cannot do" for client status-line integration.
- Install/uninstall.
...
- `vesta/integrations.py`
- `vesta/cli.py`
- `tests/test_vesta_integrations.py`
- `hub/docs/INSTALL.md`

Commands:

```powershell
python -m vesta integrate install --shell-aliases
python -m vesta integrate status
python -m vesta integrate uninstall --dry-run
```
...
```powershell
python -m vesta route "fix failing tests"
python -m vesta explain-route "ship a release"
```

## Definition Of Ready For Public Repo

Vesta is ready to push publicly when:
...
Vesta is ready for brave pre-alpha users when:

- One-command install path is documented.
- `vesta doctor` is reliable.
- `vesta install --global --shell-aliases` is idempotent.
- `vesta integrate uninstall` exists.
...
- pre-commit supports repo-local hooks, manual stages, and template-dir/global approaches; Vesta should prefer project-local hooks by default for safety: [pre-commit docs](https://pre-commit.com/).
