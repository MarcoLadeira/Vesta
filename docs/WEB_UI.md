...
`vesta:themechange`, which anything else drawn outside CSS should listen for too.

**Adding a theme** is one more palette block in `design-tokens.css`, its id in
`THEMES` in both `theme.js` and `vesta/gui_theme.py` (with its ground colour in
...
`savePref("theme", …)` writes `~/.vesta/gui_theme.json` (`vesta/gui_theme.py`),
...
- `npm run test:tokens` (`scripts/lint-web-design-tokens.mjs`) rejects raw
  colours in `styles.css` and in `vesta/assets/web/*.js`, tokens declared outside
...
- Visuals: edit `vesta/assets/web/styles.css` and `index.html`, taking every
