...
def _pyproject_duplicate_versions(path: Path) -> list[tuple[str, str]]:
    """Find app-version fields under tool.opai tables, including on Python 3.10."""
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            ...
            continue
        if not table.startswith("tool.opai") or "=" not in line or line.startswith("#"):
            continue
...
def _literal_identity_assignments(root: Path) -> list[tuple[Path, str, str]]:
    ...
    generated = (root / "opai" / "_generated_release.py").resolve()
...
def validate_release_identity(root: Path) -> tuple[IdentityDrift, ...]:
    ...
    projections = (
        (
            repository / "opai" / "__init__.py",
            "__version__",
            "APPLICATION_VERSION",
        ),
        (repository / "opai" / "__init__.py", "__release_stage__", "RELEASE_STAGE"),
        (
            repository / "opaihub" / "__init__.py",
            "__version__",
            "APPLICATION_VERSION",
        ),
    )
    ...
    for field, actual in _pyproject_duplicate_versions(pyproject):
        drifts.append(
            IdentityDrift(
                ...
                remediation="Remove the duplicate tool.opai application-version field.",
            )
        )
    for path, _name, actual in _literal_identity_assignments(repository):
        drifts.append(
            IdentityDrift(
                ...
                actual=actual,
                remediation=(
                    "Import opai._generated_release instead of editing another version."
                ),
            )
        )
