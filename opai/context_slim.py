...
AI_IGNORE_PATTERNS = [
    ...
    ".opaihub/generated/",
    ".opaihub/health/",
    ".opaihub/install-test-*/",
    ".opaihub/smoke-install-venv/",
    ".opaihub/wheelhouse/",
    ".ruff_cache/",
    ".pytest_cache/",
    ".mypy_cache/",
...
def generated_context_targets(project_root: Path) -> list[Path]:
    ...
    targets.extend(opaihub / name for name in OPAIHUB_GENERATED_TARGETS)
    if opaihub.exists():
        targets.extend(path for path in opaihub.glob("install-test-*") if path.is_dir())
    return targets
