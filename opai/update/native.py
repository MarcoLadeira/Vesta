...
class WindowsMsixAdapter:
    ...
    def verify(self, artifact: Path, candidate: UpdateCandidate) -> AdapterVerification:
        ...
        if manifest_publisher != candidate.publisher_identity:
            return AdapterVerification(False, error_category="publisher_mismatch")
        expected_application = str(candidate.native.get("application_id") or "OPai")
...
class MacOSSparkleAdapter:
    ...
    def rollback(self, last_known_good: Mapping[str, Any]) -> AdapterInstallResult:
        ...
        helper = artifact.parent / "opai-macos-recovery.sh"
        result_path = artifact.parent / "rollback-result.json"
        script = """#!/bin/bash
set -u
ARCHIVE="$1"
...
EXPECTED_BUILD="$5"
RESULT="$6"
ROOT="$(/usr/bin/mktemp -d -t opai-recovery)" || exit 1
BACKUP="${APPLICATION}.opai-recovery-backup-$$"
finish() {
  /bin/rm -rf "$ROOT"
}
trap finish EXIT
...
/usr/bin/osascript -e 'tell application "OPai" to quit' >/dev/null 2>&1 || true
for _attempt in 1 2 3 4 5; do
  /usr/bin/pgrep -x OPai >/dev/null 2>&1 || break
  /bin/sleep 1
done
/usr/bin/pkill -TERM -x OPai >/dev/null 2>&1 || true
