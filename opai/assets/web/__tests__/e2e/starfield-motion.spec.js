...
test("stars cross at their own speeds, not one shared speed", async ({
  ...
  const speeds = await page.evaluate(async () => {
    ...
    window.OPaiStarfield.CONFIG.spawnInterval = [0.02, 0.05];
    window.OPaiStarfield.CONFIG.trailLength = [10, 20];
    const canvas = document.createElement("canvas");
    canvas.style.cssText =
  ...
  const { speed } = await page.evaluate(() => window.OPaiStarfield.CONFIG);
