(function (global) {
  ...
  function state(agent) {
    const tone = agent.status === "running" ? "active" : agent.status === "completed" ? "done" : ["failed", "needs-attention", "blocked"].includes(agent.status) ? "attention" : "waiting";
    const label = agent.pending_approval ? "Needs your approval" : String(agent.status || "Queued").replace(/[-_]/g, " ");
  ...
  function activities(agent) {
    ...
    return (Array.isArray(value) ? value : value ? [value] : []).filter(Boolean).map((entry) => typeof entry === "string" ? entry : entry.title || entry.message || entry.detail || "Activity recorded");
  }
  function current(agent) {
    return activities(agent).at(-1) || agent.blocked_reason || agent.title || agent.objective || "Waiting for an assignment";
  }
  ...
  function feedHtml(objective) {
    const assignments = rows(objective.assignments);
    return '<section class="team-feed" aria-label="Team updates">' + (assignments.length ? assignments.map((a, index) => '<button type="button" class="team-update" data-team-select="' + esc(a.assignment_id) + '">' + avatar(profileIndex(a, index)) + '<span class="team-update-body"><span class="team-update-heading"><strong>' + esc(name(a, index)) + '</strong>' + state(a) + '</span><span class="team-task">' + esc(a.title || a.objective) + '</span><span class="team-current">' + esc(current(a)) + '</span></span></button>').join('') : '<p class="team-empty">Your team is getting ready. Assignments will appear here as the plan is created.</p>') + '</section>';
  }
  function panelHtml(objective, selectedId, unavailable) {
    ...
    html += '<p class="team-objective">' + esc(objective.objective) + '</p><nav class="team-roster" aria-label="Your agents">';
    html += assignments.map((a, index) => '<button type="button" class="team-person" data-team-select="' + esc(a.assignment_id) + '" aria-pressed="' + (a === selected) + '">' + avatar(profileIndex(a, index)) + '<span><strong>' + esc(name(a, index)) + '</strong><span class="team-current">' + esc(current(a)) + '</span>' + state(a) + '</span></button>').join('');
    html += '</nav>';
    if (!assignments.length) html += '<p class="team-empty">Putting your team together…</p>';
    if (selected) {
      const index = assignments.indexOf(selected);
      html += '<section class="team-detail" aria-label="Agent details"><header><h3>' + esc(name(selected, index)) + '</h3><details class="team-name-editor"><summary>Rename</summary><form data-team-rename><label>Agent name<input name="agentName" aria-label="Agent name" maxlength="40" required value="' + esc(name(selected, index)) + '"></label><button type="submit" class="btn">Save name</button></form></details></header><p class="team-task">' + esc(selected.title || selected.objective) + '</p>';
      html += connection(selected, assignments);
      ...
      if (activity.length) html += '<ol class="team-activity">' + activity.slice(-6).map((v) => '<li>' + esc(v) + '</li>').join('') + '</ol>';
      ...
      if (rows(selected.changed_files).length) html += '<details class="team-explanation"><summary>' + rows(selected.changed_files).length + ' changed files</summary><ul>' + selected.changed_files.map((v) => '<li>' + esc(typeof v === 'string' ? v : v.path) + '</li>').join('') + '</ul></details>';
      ...
      html += '<div class="team-actions">' + rows(selected.allowed_actions).filter((a) => ['approve', 'retry', 'stop'].includes(a)).map((action) => '<button type="button" class="btn" data-team-action="' + action + '">' + ({ approve: 'Approve once', retry: 'Retry', stop: 'Stop agent' })[action] + '</button>').join('') + '<button type="button" class="team-quiet" data-team-inspect>Inspect work</button></div></section>';
    }
  ...
  function mountPanel(element, objective, selectedId, options) {
    ...
    const inspect = element.querySelector('[data-team-inspect]');
    if (inspect) inspect.onclick = () => options.onInspect(selectedId);
  ...
  global.OPaiAgentsTeam = { feedHtml, panelHtml, mountPanel, name, avatar };
