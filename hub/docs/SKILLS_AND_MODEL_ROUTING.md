...
OPai 0.1.0 pre-alpha now has a small model-intelligence layer and an OPai skill library. The goal is to make every coding session start with local evidence, choose the cheapest capable route, and expose repeatable workflows through native skill discovery.
...
~/.agents/skills/opai/registry.yaml
