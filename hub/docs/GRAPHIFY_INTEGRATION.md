...
- **No vendoring.** OPai never copies graphify code into this repo. Users install the `graphifyy` package themselves; its MIT copyright notice travels with the package.
...
graphify install --platform agents   # generic ~/.agents/skills (the location OPai discovery already watches)
```
...
The `graphify-code-graph` skill (registered in `hub/skills/registry.yaml`) teaches OPai-managed agents this workflow, including the fallback to `repo-map-context` when the CLI is not installed.
