# OPai Client Demos: normal AI vs OPai-routed (#52)
...
# AFTER: route the same task through OPai
...
### Claude Code
- OPai writes `CLAUDE.md` (managed block) + global `~/.claude/CLAUDE.md`.
- Demo: open Claude Code in the repo; it reads the OPai policy and prefers
  `opai route`/`opai context pack` before sending whole files.

### Codex
- OPai writes `AGENTS.md` + `~/.agents/skills/opai/SKILL.md`.
- Demo: `opai launch codex` activates OPai, prints the `Using OPai` badge, then
  launches Codex with the policy in scope.

### Cursor
- OPai writes `.cursor/rules/opai.mdc` and `.cursorignore`.
...
### Cline
- OPai writes `.clinerules/opai.md` and `.clineignore`.
