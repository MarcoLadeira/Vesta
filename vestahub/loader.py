...
def packaged_hub_root() -> Path:
    return Path(str(resources.files("vestahub").joinpath("data", "hub"))).resolve()


def hub_root(start: Path | None = None) -> Path:
    configured = os.environ.get("VESTA_HUB_ROOT")
    if configured:
...
def load_registry(path: Path) -> Any:
    ...
    try:
        ...
        raise RegistryLoadError(
            ...
            "Repair the installation with `python -m pip install vesta`."
        ) from exc
    try:
