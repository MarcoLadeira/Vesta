...
same rules (unlike per-project state under the gitignored `.vestahub/`):

```sh
vesta team init --team acme --profile team-safe   # writes vesta-team-policy.yaml
vesta team apply                                  # set local policy from it
```

`vesta-team-policy.yaml` pins the profile, budgets, paid posture, and an optional
...
- name: Vesta governance gate
  run: python -m vesta policy check
```
...
`.vestahub/audit/`. Editing or deleting a middle entry breaks the hash chain.
...
vesta guard verify .vestahub/evidence/release_preflight.json
```

Teams share one key out-of-band via `VESTA_SIGNING_KEY` (e.g. a CI secret); solo
users get an auto-generated local key under `.vestahub/keys/` (gitignored).
