# Vesta Effectiveness Benchmarks

Vesta benchmarks prove the product promise with local evidence: Vesta should be
...
VestaBench also ships a versioned, hermetic coding-task suite. It copies four
small repositories into temporary directories and sends each task through the
real Vesta build pipeline, then verifies the resulting Git diff, expected source
content, run checkpoint, and local unit tests. The suite covers a bug fix, a
feature, a refactor, and a test addition.

```sh
vesta hub vestabench parity
vesta hub vestabench parity --task bugfix --no-write
vesta hub vestabench parity --format markdown
```
...
`$0.00`. This validates Vesta's context/edit/checkpoint/test contract; it does
not measure model intelligence. JSON, Markdown, and HTML evidence is written to
`.vestahub/benchmarks/vestabench/parity/` unless `--no-write` is supplied.

Paid-system comparisons are never launched by this command. To compare results,
capture another system separately, copy
`vestahub/data/hub/benchmarks/parity/baseline.example.json`, enter the observed
...
`N/A`; Vesta does not invent comparison values.

## Local suite

The MVP suite compares normal AI use against Vesta-routed use across:
...
tier from the local cost model. The Vesta run uses the existing local-first router
and measures the compact context payload Vesta would give an agent.
...
- Vesta governance tasks for policy, audit, release, dependency, and mobile gates.

The max suite reports an `vesta_effectiveness_index` from `0` to `100` and a
`leaderboard_grade`. A local `A+` means Vesta is maxing its control-plane metrics
locally; it does **not** mean Vesta has submitted to SWE-bench, Terminal-Bench, or
Aider.
...
This is the core product wedge: Vesta does not just claim cost control; it can
...
under `.vestahub/benchmarks/promptfooconfig.yaml`. The export uses task ids and
...
agent evaluations, assertions, scoring, and cost checks. Vesta does not invoke it
...
```text
Vesta reduced context by 12x and avoided 90% of paid calls on the local benchmark suite.
```
