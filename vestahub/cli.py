...
def cmd_vestabench(args: argparse.Namespace) -> int:
    from .vestabench import (
    ...
    root = _project(args.project)
    if args.vestabench_command == "run":
        report = run_vestabench(root, write=not args.no_write)
        print_json(report)
        return 0 if report["totals"]["passed"] == report["totals"]["total"] else 1
    if args.vestabench_command == "dashboard":
        print_json({"path": str(build_vestabench_dashboard(root)), "format": "html"})
    elif args.vestabench_command == "history":
        print_json(read_vestabench_history(root, limit=args.limit))
    elif args.vestabench_command == "parity":
        try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="op-hub", description="Vesta / Vesta Hub registry CLI"
    )
    ...
    p = sub.add_parser("vestabench")
    vestabench_sub = p.add_subparsers(dest="vestabench_command", required=True)
    ob = vestabench_sub.add_parser("run")
    ob.add_argument("--no-write", action="store_true")
    ob.set_defaults(func=cmd_vestabench)
    ob = vestabench_sub.add_parser("dashboard")
    ob.set_defaults(func=cmd_vestabench)
    ob = vestabench_sub.add_parser("history")
    ob.add_argument("--limit", type=int, default=10)
    ob.set_defaults(func=cmd_vestabench)
    ob = vestabench_sub.add_parser(
