...
def _classify_gemini(agent: str, argv: Sequence[str]) -> InvocationPlan:
    from .provider_adapters import vesta_mode_for_gemini_approval
    ...
    while index < len(argv):
        ...
        if token in {"-p", "--prompt"}:
            ...
            mapped = vesta_mode_for_gemini_approval(str(approval_mode or ""))
            if mapped is None:
            ...
            mapped = vesta_mode_for_gemini_approval(approval_mode)
            if mapped is None:
                return _passthrough(agent, argv, "unsupported_approval_mode")
            mode = mapped
            index += 1
