...
Runs live under ``.vestahub/agent/background/`` and survive restarts. The queue
...
# #612: every table below is a *projection* of vestahub/lifecycle_schema.json,
    ...
    def start(self, run_id: str) -> threading.Thread:
        ...
        with self._lock:
            ...
            thread = threading.Thread(
                target=self._execute, args=(run,), daemon=True, name=f"vesta-bg-{run_id}"
            )
...
def runs_owned_by_a_live_process(project_root: Path) -> frozenset[str]:
    ...
    and ``vestahub/cli.py`` passes nothing at all. The journal knows better,
