...
_VESTA_STATE_DIRS = frozenset({".vestahub", ".opcoding"})
...
def _is_vesta_state_path(relative: str) -> bool:
    parts = PurePosixPath(relative).parts
    return bool(parts) and parts[0].casefold() in _VESTA_STATE_DIRS
...
def _bypass_fingerprint(
    ...
    *,
    ...
    digest = _digest(
        b"vesta-repo-fingerprint-v3-bypass",
        reason.encode("utf-8"),
        status_basis,
...
def assess_repo_fingerprint(
    ...
    *,
    ...
    status_parts = [b"vesta-repo-fingerprint-v3", b"git", head.strip()]
    ...
    for code, raw_paths in parsed:
        ...
        if all(_is_vesta_state_path(path) for path in normalized_paths):
            continue
        status_parts.extend(
            [
                code,
        ...
        candidates.update(
            path for path in normalized_paths if not _is_vesta_state_path(path)
        )
