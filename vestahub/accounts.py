...
from vesta.model_registry import models_for as _models_for
...
def _hidden_run(
    ...
    *,
    ...
    :func:`vestahub.proc.provider_child_env` — parent AI-session variables must
...
# vesta.model_registry (#170) — no more parallel tuples drifting against the
...
def claude_hook_settings_path() -> Path:
    ...
    return Path(tempfile.gettempdir()) / "vesta" / _CLAUDE_HOOK_SETTINGS_NAME
...
def _guard_int_env(name: str, default: int) -> int:
    ...
    Used by the #648 convergence guard (`VESTA_NO_PROGRESS_STEP_BUDGET` as the
    absolute exploration ceiling, `VESTA_NO_PROGRESS_PATIENCE` for stagnation,
    `VESTA_NO_PROGRESS_SECONDS` for wall clock). ``0`` disables that check.
    """
...
class AccountRunner:
    ...
    def __init__(self, account_id: str, cli_path: str, *, model: str | None = None):
        ...
        from vesta.model_registry import resolve_id
        ...
        *,
        ...
        if self.account_id == "claude":
            ...
            if selected_mode == "full-auto":
                ...
                # child carries in VESTA_AUTONOMY so the PreToolUse hook in the
    ...
    def complete(
        ...
        *,
        ...
        # this CLI's auth or model selection (see vestahub.proc).
    ...
    def stream(
        ...
        *,
        ...
        """
        from vesta.activity import ActivitySession, make_event
        ...
        # `vestahub/progress_evidence.py`, and `tool_loop.py` (local and free
        ...
        exploration_ceiling = _guard_int_env("VESTA_NO_PROGRESS_STEP_BUDGET", 400)
        stagnation_patience = _guard_int_env("VESTA_NO_PROGRESS_PATIENCE", 12)
        ...
        no_progress_seconds = _guard_int_env("VESTA_NO_PROGRESS_SECONDS", 600)
