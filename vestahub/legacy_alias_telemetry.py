...
``vestahub/legacy_status.py`` already counts legacy reads and writes and
already publishes the gate they must clear —
``REMOVAL_GATE = "zero authoritative legacy reads and writes for one
supported release"`` — through :func:`~vestahub.legacy_status.legacy_status_usage`.
...
def coverage_report() -> dict[str, Any]:
    ...
    return {
        "report": "vesta-legacy-alias-usage",
