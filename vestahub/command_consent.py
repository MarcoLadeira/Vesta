...
# for (#818 AC8). Defined here rather than in ``vestahub.proc`` because proc
...
RUN_ENV = "VESTA_RUN_ID"

# The run this turn has armed a grant for. Set by :func:`begin_turn` and read
# by ``vestahub.proc.provider_child_env`` when it builds a child environment, so
...
_CURRENT_RUN: ContextVar[str] = ContextVar("vesta_command_consent_run", default="")
...
def consent_dir() -> Path:
    ...
    plumbing. ``VESTA_COMMAND_CONSENT_DIR`` overrides it for tests.
    """

    override = str(os.environ.get("VESTA_COMMAND_CONSENT_DIR", "") or "").strip()
    if override:
        return Path(override)
    return Path(tempfile.gettempdir()) / "vesta" / "consent"
...
def resolve_run(explicit: str | None = None) -> str:
    ...
    ``VESTA_RUN_ID``, exported by ``provider_child_env``. In-process callers
...
def grant_belongs_to(grant_run: Any, caller_run: Any) -> bool:
    ...
    **Refused only on a positive mismatch**: both sides name a run, and the
    ...
    hook. Whether ``VESTA_RUN_ID`` survives the middle hop is a third party's
