...
IF-THEN library that turns a :class:`~vestahub.failure_envelope.FailureEnvelope`
...
RECIPES: tuple[RecoveryRecipe, ...] = (
    RecoveryRecipe(
        ...
        governance=RecipeGovernance(owner="vesta-core", expires_on="2027-01-01"),
    ),
    RecoveryRecipe(
        ...
        caps=_ONE_CHEAP_ATTEMPT,
        allowed_operations=("vesta_github_connector",),
...
def library_coverage() -> dict[str, Any]:
    ...
    return {
        "report": "vesta-recovery-recipe-coverage",
        "families": {
            family.value: {
