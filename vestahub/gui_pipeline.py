...
def _command_approval(result: Mapping[str, Any] | None) -> dict[str, str] | None:
    ...
    carrying ``{"command", "reason"}``. ``vestahub.ask.run_explicit_model``
...
def build_savings_receipt(
    ...
    *,
    ...
    integrity = str(cost_integrity or "complete")
    if integrity == "unreconciled":
        savings = 0.0
        paid_avoided = False
...
_JOURNAL_RUN: ContextVar[dict[str, Any] | None] = ContextVar(
    "_vesta_journal_run", default=None
)
...
def _handle_gui_message(
    ...
    *,
    ...
    from vesta.activity import derived_id, new_id
    ...
    if resume_context:
        from vesta.gui_recents import normalize_resume_execution_context
    ...
    def _decorate(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if status in _DEAD_END_STATUSES and contract.allow_provider_fallback:
            with contextlib.suppress(Exception):  # noqa: BLE001 - never fail a turn
                from vesta import app_state as _app_state
    ...
    if selected_model in usage_limits and not allow_limit:
        ...
        provider = parts[1] if len(parts) > 1 else "vesta"
