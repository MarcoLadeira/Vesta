...
# decision object so a claimed completion can be verified (vestahub/tool_loop.py).
...
_TOOL_LOOP_PROTOCOL = (
    ...
    '{"vesta_decision_version": 1, "state": "completed", "summary": "<what you '
...
class FreeAPIRunner(OpenAICompatibleRunner):
    ...
    def _thinking_requested(self) -> bool:
        ...
        ``complete_with_tools`` to stamp :class:`~vestahub.tool_loop.ChatTurn`
    ...
    def complete_with_tools(
        ...
        *,
        ...
        Thin transport adapter over :class:`~vestahub.tool_loop.ToolLoopController`:
...
class PaidAPIRunner(FreeAPIRunner):
    ...
    responsibility (``vestahub/tool_loop.py``'s ``ToolProtocolAtom`` and
    ...
    def _cost_for(self, usage: dict[str, Any]) -> tuple[float | None, str]:
        ...
        if measurement == "estimated_stale":
            ...
            # (vesta/app_state.py) has no such closed set and keeps the more
