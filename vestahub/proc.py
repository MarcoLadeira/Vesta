...
``vestahub.accounts`` already applies this to the provider CLIs it launches;
...
# Marker exported into every provider CLI's environment. Any `vesta` process an
...
# recursive self-invocation (F12). See vesta.cli._refuse_if_nested_agent_session.
AGENT_SESSION_ENV = "VESTA_AGENT_SESSION"
...
COMMAND_CONSENT_DIR_ENV = "VESTA_COMMAND_CONSENT_DIR"
...
def provider_child_env(
    ...
    *,
    ...
    Every child env also carries ``VESTA_AGENT_SESSION`` so a nested `vesta`
    invocation from inside the agent can detect and refuse recursion (F12), and
    ``VESTA_COMMAND_CONSENT_DIR`` so the PreToolUse hook the child launches reads
