...
def _catalog_kind(record: dict[str, Any]) -> str:
    ...
    ``vesta.model_registry.PAID_DIRECT_PROVIDERS`` — the one place that
...
def _paid_direct_providers() -> frozenset[str]:
    from vesta.model_registry import PAID_DIRECT_PROVIDERS
...
def vesta_mode_for_gemini_approval(approval_mode: str) -> str | None:
...
class ProviderAdapter:
    ...
    def normalize_event(self, event: dict[str, Any] | str) -> dict[str, Any]:
        ...
        from vesta.activity import parse_claude_line, parse_codex_line
