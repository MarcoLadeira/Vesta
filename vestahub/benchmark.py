"""Offline Vesta effectiveness benchmarks.

The benchmark suite compares a plain "send the task to a strong model" baseline
against Vesta's existing local-first router. It is local and fixture-backed by
default: no provider calls, no raw prompt storage, and only privacy-safe hashes
are persisted under .vestahub/benchmarks/.
"""
...
VALIDATION_LADDER: list[dict[str, str]] = [
    {
        "id": "local",
        "status": "implemented",
        "purpose": "Fast fixture-backed proof that Vesta reduces context, paid calls, and unsafe escalation.",
    },
    {
        "id": "promptfoo",
        "status": "export_ready",
...
MAX_TASKS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "category": "security_review",
        "benchmark_alignment": ["terminal-bench", "vesta-governance"],
    ...
    },
    {
        ...
        "category": "security_review",
        "benchmark_alignment": ["promptfoo", "vesta-governance"],
    ...
    },
    {
        ...
        "category": "security_review",
        "benchmark_alignment": ["vesta-governance"],
    ...
    },
    {
        ...
        "category": "dependency_update",
        "benchmark_alignment": ["swe-bench-pro", "vesta-governance"],
    ...
    return {
        ...
        "max": {
            ...
            "description": (
                ...
                "Terminal-Bench, Aider Polyglot, promptfoo, and Vesta governance signals."
            ),
            "tasks": [
                {
                    "id": task["id"],
...
def _selected_modes(mode: str) -> list[str]:
    if mode not in {"baseline", "vesta", "both"}:
        msg = "mode must be one of: baseline, vesta, both"
        raise ValueError(msg)
    if mode == "both":
        return ["baseline", "vesta"]
    return [mode]
...
def _vesta_local_command_count(decision: dict[str, Any]) -> int:
...
def _vesta_result(
    ...
    return {
        ...
        "local_commands_used": _vesta_local_command_count(decision),
...
def _task_result(
    ...
    *,
    ...
    if "vesta" in selected:
        vesta = _vesta_result(project_root, task, cost_model)
        if "baseline" in selected:
            vesta["cloud_calls_avoided"] = max(
                0, baseline["paid_model_calls"] - vesta["paid_model_calls"]
            )
        modes["vesta"] = vesta
    return {
        "task_id": task_id,
...
def _score(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    vesta_context = _sum_mode(results, "vesta", "context_bytes")
    baseline_paid = _sum_mode(results, "baseline", "paid_model_calls")
    vesta_paid = _sum_mode(results, "vesta", "paid_model_calls")
    baseline_cost = _sum_mode(results, "baseline", "estimated_cost_usd")
    vesta_cost = _sum_mode(results, "vesta", "estimated_cost_usd")
    time_to_evidence = _sum_mode(results, "vesta", "time_to_evidence_seconds")
    risk_blocks = int(_sum_mode(results, "vesta", "risk_events_blocked"))
    human_interventions = int(_sum_mode(results, "vesta", "human_interventions"))
    context_ratio = _ratio(baseline_context, vesta_context)
    paid_ratio = _ratio(baseline_paid, vesta_paid)
    cost_ratio = _ratio(baseline_cost, vesta_cost)
    ...
    return {
        "context_reduction_ratio": _ratio(baseline_context, vesta_context),
        ...
        "paid_calls_avoided": int(max(0, baseline_paid - vesta_paid)),
        "context_bytes_saved": int(max(0, baseline_context - vesta_context)),
        "estimated_cost_saved_usd": round(max(0.0, baseline_cost - vesta_cost), 6),
        "vesta_effectiveness_index": index,
...
def claim_readiness(score: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": status,
        "public_claim": (
            f"Vesta reduced context by {context_ratio:g}x and avoided "
...
def compare_benchmark_reports(
    ...
    return {
        "kind": "vesta-benchmark-comparison",
        "ok": not regressions,
...
def export_promptfoo_config(
    ...
    lines = [
        "# Generated by Vesta. Provider-backed runs are opt-in.",
        "description: Vesta coding-agent benchmark handoff",
        "providers:",
        "  - id: echo",
        "prompts:",
        "  - 'Run Vesta benchmark task {{task_id}} with your coding-agent harness.'",
        "tests:",
    ]
    for task in tasks:
    ...
    lines.extend(
        [
            "metadata:",
            "  vesta_privacy: raw prompts are not exported",
...
def render_benchmark_html(report: dict[str, Any]) -> str:
    ...
    return (
        "<!doctype html>\n"
        '<html><head><meta charset="utf-8"><title>Vesta Benchmark Report</title>'
