...
from vesta._generated_release import PUBLISHED_TAG
from vesta.asset_identity import REQUIRED_WEB_ASSETS, asset_manifest
...
_DEVELOPMENT_OVERRIDE_PATTERN = re.compile(
    r"\b(?:VESTA_HUB_ROOT|LOCAL_MODEL_URL|LOCAL_MODEL_NAME|PYTHONPATH)\s*=",
    flags=re.IGNORECASE,
)
...
def deployment_specs(project_root: Path, output_dir: Path) -> DeploymentSpecs:
    ...
    gui_args = [
        "--output-filename=Vesta",
        "--include-package=vesta",
        "--include-package=vestahub",
        "--include-package=opcoding",
        _data_dir_arg(root, Path("vesta") / "assets" / "fonts"),
        _data_dir_arg(root, Path("vesta") / "assets" / "web" / "icons"),
        _data_file_arg(root, Path("vesta") / "assets" / "vesta-icon.png"),
        _data_file_arg(root, Path("vesta") / "assets" / "vesta-mascot.png"),
        _data_dir_arg(root, Path("vestahub") / "data"),
    ]
    gui_args.extend(
        _data_file_arg(root, Path("vesta") / "assets" / "web" / asset)
...
def render_pyside_deploy_spec(
    ...
    icon = icon or root / "vesta" / "assets" / "vesta-icon.png"
    if not icon.is_file():
...
def smoke_commands(
    ...
    cli = _component_executable(root, "cli", "vesta")
    gui = _component_executable(root, "gui", "Vesta")
...
def _hash_entry(bundle: Path, path: Path) -> str:
    if path.is_symlink():
        ...
        return hashlib.sha256(
            b"vesta-artifact-symlink-v1\0" + str(path.readlink()).encode("utf-8")
        ).hexdigest()
    ...
    if artifact_identity is None:
        artifact_identity = artifact_identity_payload(
            ...
            assets=asset_manifest(
                Path(__file__).resolve().parents[1] / "vesta" / "assets"
            ),
