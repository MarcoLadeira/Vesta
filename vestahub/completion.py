...
# Typed provider error codes (vesta.provider_contract.ERROR_CODES) → failure class
...
_INTERNAL_FAILURE_STATUSES = frozenset({"runner_error", "internal_error", "vesta_error"})
...
def required_test_kinds(policy: Any) -> bool:
    ...
    Reads a resolved :class:`~vestahub.verification_policy.VerificationPolicy`
...
MEASURED_EVIDENCE_KEYS = frozenset(
    {
        ...
        # from the provider's own tool stream by vesta.activity. Allowlisted
    ...
    if isinstance(verification, Mapping):
        for check in verification.get("checks") or ():
            ...
            if _normalized(check.get("kind")) not in {"tests", "test", "unit"}:
                continue
            if check.get("exit_status") == 0 and check.get("observed_by") == "vesta":
                return True
    return False
...
_OUTPUT_CLAIM = re.compile(
    ...
    r"|\b(?:i|vesta)\s+(?:have\s+)?(?:retrieved|fetched|printed|displayed|ran|run)\b"
