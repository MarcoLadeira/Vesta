...
`vestahub validate` already gates the tools, agents, workflows, mcp_servers, and
...
orphan detection, and no safety rule. `vestahub skills doctor` checked only that
each file exists.
...
def validate_skills(hub_root: Path) -> dict[str, Any]:
    ...
    Matches the shape `vestahub validate` already emits for the other registries
