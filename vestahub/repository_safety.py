...
def _is_vesta_state_path(value: str) -> bool:
    normalized = value.replace("\\", "/").strip("/")
    return normalized == ".vestahub" or normalized.startswith(
        (".vestahub/", ".opcoding/", ".opcoding")
    )
...
def _dedupe(values: list[str]) -> tuple[str, ...]:
    return tuple(
        dict.fromkeys(
            value for value in values if value and not _is_vesta_state_path(value)
        )
    )
...
def classify_dirty_state(
    ...
    *,
    ...
    vesta_owned_paths: Iterable[str] = (),
) -> DirtyAssessment:
    ...
    owned = tuple(_normal_path(item) for item in vesta_owned_paths)
    owned_clean = tuple(item for item in owned if item is not None)

    def is_vesta_owned(path: PurePosixPath) -> bool:
        return any(_paths_overlap(path, prefix) for prefix in owned_clean)

    foreign_dirty = tuple(path for path in dirty_clean if not is_vesta_owned(path))
    overlaps = tuple(
        path.as_posix()
