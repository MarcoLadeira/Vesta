...
def attachments_dir(root: Path) -> Path:
    """Where images for this workspace live: ``.vestahub/attachments``.

    Inside the workspace because a context reference is workspace-relative,
    and under ``.vestahub`` because that directory is already Vesta's and
    already ignored by git -- an attachment must never turn up in a diff.
    """

    return Path(root).expanduser().resolve() / ".vestahub" / ATTACHMENT_DIRNAME
