...
def observe_changes(root: Path, base_sha: str) -> dict:
    ...
    untracked = b"\0".join(
        p for p in untracked.split(b"\0") if not p.startswith(b".vestahub/")
    )
    paths = sorted(
...
def worker_command(request: Path, response: Path) -> list[str]:
    from vesta.bootstrap import _packaged_runtime, _runtime_executable

    command = (
        [_runtime_executable(), "--vesta-objective-worker"]
        if _packaged_runtime()
        else [sys.executable, "-m", "vestahub.objective_worker"]
    )
...
def run_worker_process(
    ...
    env = dict(os.environ)
    env["VESTA_COMMAND_CONSENT_DIR"] = str(directory / "consent")
...
class ObjectiveExecutor:
    ...
    def __init__(
        ...
        self.worktree_root = (
            ...
            else Path.home() / ".vestahub" / "worktrees"
        )
