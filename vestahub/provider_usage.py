...
budgets (``vestahub.usage``) and from Credits & Balance
(``vestahub.provider_balance``): *how much of each provider's own rate/usage
...
* ``vesta-tracked`` — Vesta's own local count of calls/tokens in the applicable
...
def usage_snapshot(
    ...
    *,
    ...
    vesta_tracked = _vesta_tracked(events, provider, model, now=ts)
