...
# run_mode values must match vestahub.gui_preferences.MODES.
...
def describe_controls(run_mode: str | None, focus: str | None) -> dict[str, Any]:
    ...
    *Task focus* (intent persona), and the derived *Agent mode* (per-message
    ...
    uses per message (``vestahub.agent_policy.resolve_agent_policy``) called
    ...
    """
    from vesta.gui_permissions import is_read_only, permissions_for
...
def plan_mode_selection(selected_mode: str, prefs: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from vestahub.autonomy import is_full_auto_pinned
