...
front-end (``vesta/assets/web/``). All of Vesta's logic stays in Python and is
...
from vesta.gui_nav import DEFAULT_VIEW, group_collapsed, nav_groups
from vesta.gui_permissions import permission_summary, permissions_for
from vesta.gui_prompts import categories_present, filter_prompts, find_prompt
...
# process start. Off unless VESTA_STARTUP_TRACE is set, in which case boot marks
# its stages and the front-end flushes an "interactive" mark after first paint.
from vestahub.startup_trace import StartupTrace, trace_enabled, trace_path  # noqa: E402
from vestahub.boundary_errors import safe_detail  # noqa: E402
...
def _state_fingerprint(root: Path) -> tuple:
    ...
    # Each public path helper validates and resolves ``.vestahub`` independently.
...
def _models(
    ...
    from vesta.model_overrides import load_overrides, overrides_report_payload
    from vesta.model_registry import ACCOUNT_PROVIDERS
...
def _workspace(root: Path) -> dict[str, Any]:
    from vestahub.repo_context import repository_safety_surface, save_active_repo
    ...
    from vestahub.build_loop import load_app_manifest
...
def _inspector(root: Path, sel: dict[str, Any]) -> dict[str, Any]:
    ...
    if run_mode in {"safe-auto", "full-auto"}:
        try:
            from vestahub.github_connector import github_readiness
...
def _resume_payload(root: Path) -> dict[str, Any]:
    ...
    from vestahub.checkpoints import list_run_checkpoints
    from vestahub.workflow_state import load_workflow_state
...
def save_page_preference(root: Path, key: str, value: str) -> None:
    ...
    goes to its own store (``vesta.gui_theme``) so that choosing light mode in
...
def boot_payload(root: Path, *, initial_task: str | None = None) -> dict[str, Any]:
    ...
    from vestahub.gui_preferences import MODES, load_gui_preferences
    from vestahub.workflow_state import load_workflow_state
    from vestahub.process_tree import objective_runtime_support

    from vestahub.autonomy import MODE_LABELS, resolve_startup_mode
    ...
    with contextlib.suppress(Exception):  # noqa: BLE001
        ...
        import vesta as _vesta

        from vesta.update.running_build import prime as _prime_running_build

        _prime_running_build(_Path(_vesta.__file__).parent / "assets")
    ...
    payload = {
        ...
        "prefs": {
            ...
            # The theme is app-wide (vesta.gui_theme), not a project preference:
...
def start_fresh_payload(root: Path) -> dict[str, Any]:
    ...
    from vesta.gui_recents import clear_thread
...
def clear_history_payload(root: Path) -> dict[str, Any]:
    ...
    from vesta.gui_recents import clear_recents, load_thread
    from vestahub.build_loop import scrub_build_log_requests
...
def _beat_lease(root: Path, request_id: str) -> None:
    ...
    from vesta.gui_recents import refresh_thread_lease
...
def _assistant_presentation(result: Any, attributed_files: Any) -> dict[str, Any]:
    ...
    from vesta.gui_recents import _clean_presentation
...
def _persist_turn_result(
    ...
    *,
    ...
    from vesta.gui_recents import finish_thread_turn, thread_status_for_result
...
def github_status_payload() -> dict[str, Any]:
    ...
    from vestahub.github_connector import github_status
...
def github_connect_payload(token: str, *, http: Any = None) -> dict[str, Any]:
    ...
    """
    from vestahub.github_connector import connect_github, github_readiness
...
def github_set_push_payload(enabled: bool) -> dict[str, Any]:
    ...
    from vestahub.github_connector import set_push_allowed
...
def github_disconnect_payload() -> dict[str, Any]:
    ...
    from vestahub.github_connector import disconnect_github
...
def settings_payload(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        overview = {}
    models = _models(root, discover_local=False)
    from vestahub.accounts import codex_config_issue, provider_connection_doctor
    from vestahub.credentials import credential_statuses
    from vestahub.github_connector import github_status
...
def provider_balances_payload(
    ...
    *,
    ...
    from vestahub.provider_balance import balance_overview
...
def _usage_providers(root: Path, models: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    from vestahub.credentials import CredentialStore
    from vestahub.free_models import FREE_MODEL_SPECS
    from vestahub.provider_usage import USAGE_MODELS
...
def provider_usage_payload(
    ...
    *,
    ...
    from vestahub.provider_usage import usage_overview
...
def _recents(root: Path) -> list[str]:
    from vesta.gui_recents import load_recents
...
def _conversations(root: Path) -> list[dict[str, Any]]:
    ...
    from vesta.gui_recents import list_conversations
...
def _brand() -> dict[str, str]:
    from vesta.brand import boot_brand
    ...
    *,
    ...
    class Bridge(QtCore.QObject):
        ...
        def __init__(self, window) -> None:
            ...
            from vesta.update.scheduler import UpdateScheduler
        ...
        def resumeSession(self) -> str:
            ...
            from vesta.gui_recents import resume_execution_context
        ...
        def reportIllegalTransition(self, from_state: str, to_state: str) -> None:
            ...
            """
            from vestahub.lifecycle_diagnostics import record_illegal_transition
        ...
        def reviewDiff(self, path: str, decision: str) -> str:
            from vestahub.diff_review import record_diff_decision
        ...
        def testProvider(self, provider: str) -> str:
            ...
            if str(provider or "").strip().lower() == "github":
                from vestahub.github_connector import verify_github_connection
        ...
        def saveModelOverrides(self, payload_json: str) -> str:
            ...
            from vesta.model_overrides import save_override_payload
        ...
        def setProviderBalance(self, provider: str, amount: str, currency: str) -> str:
            ...
            from vestahub.provider_balance import set_manual_balance
        ...
        def repairCodexConfig(self) -> str:
            from vestahub.accounts import repair_codex_config
        ...
        def startProviderLogin(self, provider: str, request_id: str) -> None:
            ...
            from vestahub.accounts import interactive_provider_login
        ...
        def grantFreeConsent(self, model_id: str) -> str:
            ...
            from vestahub.gui_preferences import grant_free_consent
        ...
        def pinFullAuto(self) -> str:
            ...
            from vestahub.gui_preferences import pin_full_auto
        ...
        def unpinFullAuto(self) -> str:
            ...
            from vestahub.autonomy import resolve_startup_mode
            from vestahub.gui_preferences import unpin_full_auto
        ...
        def attachImage(self, data: str, name: str) -> str:
            ...
            trusted here -- vestahub.attachments does the sniffing, the capping
            and the containment. This slot only routes, and turns a rejection
            into a message the composer can show.
            """

            from vestahub.attachments import AttachmentError, store_image
            from vestahub.command_runner import redact
        ...
        def inspectObjectiveArtifact(self, payload_json: str) -> str:
            from vesta.agents_bridge import inspect_objective_artifact
            from vestahub.worktree_leases import WorktreeLeaseError
        ...
        def saveRecent(self, text: str) -> None:
            from vesta.gui_recents import add_recent
        ...
        def loadConversation(self, conversation_id: str) -> str:
            ...
            from vesta.gui_recents import load_conversation
    ...
    if artifact_smoke_result is not None:
        ...
        def poll_artifact_page() -> None:
            window.view.page().runJavaScript(
                ...
                "bridgeBooted: Boolean(window.__vesta && window.__vesta.state && "
                "window.__vesta.state.boot)"
                "})",
    ...
    from vesta.gui_identity import apply_window_identity
