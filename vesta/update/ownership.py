...
def _distribution_installer() -> str:
    ...
    try:
        text = metadata.distribution("vesta").read_text("INSTALLER") or ""
...
def describe_ownership(
    ...
    if f"{os.sep}cellar{os.sep}" in interpreter or "/opt/homebrew/" in interpreter:
        return UpdateOwnership(
            ...
            remediation="Update with `brew upgrade vesta`.",
            self_updatable=False,
        )
    if installer:
        return UpdateOwnership(
            ...
            remediation=f"Update with `{installer} install --upgrade vesta`.",
            self_updatable=False,
        )
    return UpdateOwnership(
        owner="unknown",
...
def running_identity() -> dict[str, str]:
    ...
    build = ""
    try:
        from vesta._generated_release import APPLICATION_BUILD_ID  # type: ignore

        build = str(APPLICATION_BUILD_ID)
    except Exception:  # noqa: BLE001 - development builds carry no build id
        build = "development"
    return {"version": str(getattr(vesta, "__version__", "")), "build_id": build}
