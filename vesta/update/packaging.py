...
from vesta.compatibility import validate_compatibility_coordinates
    ...
    *,
    ...
    alias_extension = ElementTree.SubElement(
        ...
        f"{{{uap3}}}Extension",
        {
            ...
            "Executable": "cli\\vesta.exe",
            "EntryPoint": "Windows.FullTrustApplication",
        },
    )
    aliases = ElementTree.SubElement(alias_extension, f"{{{uap3}}}AppExecutionAlias")
    ElementTree.SubElement(
        aliases, f"{{{desktop}}}ExecutionAlias", {"Alias": "vesta.exe"}
    )
...
def prepare_msix_layout(
    ...
    *,
    ...
    if (
        not (source / "gui" / "Vesta.exe").is_file()
        or not (source / "cli" / "vesta.exe").is_file()
    ):
...
def prepare_macos_sparkle_bundle(
    ...
    *,
    ...
    cli_candidates = [
        ...
        if path.is_file() and path.name == "vesta"
    ]
