...
def _client_specs(project_root: Path, home: Path) -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "global_files": [home / ".agents" / "skills" / "vesta" / "SKILL.md"],
        },
        {
            "id": "copilot",
            ...
            "global_files": [
                vesta_home(home) / "integrations" / "copilot-instructions.md"
            ],
        },
        {
            "id": "gemini",
            ...
            "global_files": [
                vesta_home(home) / "integrations" / "gemini-instructions.md"
            ],
        },
        {
            "id": "cursor",
            "label": "Cursor",
            "project_files": [project_root / ".cursor" / "rules" / "vesta.mdc"],
            "global_files": [],
        },
        {
            "id": "cline",
            "label": "Cline",
            "project_files": [project_root / ".clinerules" / "vesta.md"],
            "global_files": [],
        },
    ]


def _has_vesta_block(path: Path) -> bool:
    if not path.exists():
        return False
    try:
...
def detect_stale_paths(project_root: Path, home: Path | None = None) -> dict[str, Any]:
    ...
    manifest = vesta_home(user_home) / "global.json"
    if manifest.exists():
        try:
    ...
    return {
        "ok": not issues,
        "issues": issues,
        "vesta_source": str(source),
        "vesta_source_present": source_present,
    }
