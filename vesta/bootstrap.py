...
def _embedded_build_exists(root: Path) -> bool:
    return any(
        ...
        for candidate in (
            root / "vesta" / "_embedded_build.json",
...
def _repair_command(mode: str, *, desktop: bool = False) -> str:
    ...
    return (
        ...
        f'"vesta=={APPLICATION_VERSION}"` and retry.'
    )
...
def detect_startup_context(
    *,
    ...
    if is_source:
        ...
        try:
            distribution_version = distribution_lookup("vesta")
            ...
            if _embedded_build_exists(root):
                ...
                raise BootstrapFailure(
                    ...
                    component="vesta-distribution-metadata",
                    message=(
    ...
    if is_source:
        ...
        raise BootstrapFailure(
            ...
            remediation=(
                f'Run `python -m pip install "vesta=={APPLICATION_VERSION}"` and '
                "launch the installed `vesta` command."
            ),
...
def _validate_persisted_project_schema(
    ...
    *,
    ...
    if not isinstance(supported, int) or isinstance(supported, bool):
        return
    state_path = _project_root(arguments) / ".vestahub" / "project.json"
    try:
...
def _module_failure(exc: BaseException, mode: str) -> BootstrapFailure | None:
    ...
    if (
        ...
        and type(exc).__module__ == "vestahub.loader"
    ):
...
def _run(
    ...
    *,
    ...
    try:
        internal_entries = {
            "--vesta-objective-worker": ("vestahub.objective_worker", "main"),
            "--vesta-objective-guardian": ("vestahub.objective_guardian", "main"),
            "--vesta-objective-child": ("vestahub.objective_guardian", "child_main"),
        }
