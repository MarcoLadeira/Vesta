...
def publish_status(root: Path) -> dict[str, Any]:
    ...
    ready = (
        ...
        and scripts.get("op") == "vesta.bootstrap:cli_main"
        and scripts.get("vesta") == "vesta.bootstrap:cli_main"
    )
    next_steps: list[str] = []
    if not is_repo_root:
    ...
    if scripts.get("op") != "vesta.bootstrap:cli_main":
...
def write_publish_status(root: Path) -> Path:
    ...
    path = root.expanduser().resolve() / ".vestahub" / "publish-status.json"
