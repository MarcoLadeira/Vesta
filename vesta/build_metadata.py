...
def _existing_payload(source_root: Path) -> dict[str, object]:
    path = source_root / "vesta" / "_embedded_build.json"
...
def build_metadata_payload(
    *,
    ...
    raw_build_id = values.get("VESTA_BUILD_ID")
    ...
    if build_id != "unknown" and _COMMIT_SHA.fullmatch(build_id) is None:
        raise BuildMetadataError("VESTA_BUILD_ID must be an exact lowercase commit SHA")
    channel = str(
        values.get("VESTA_RELEASE_CHANNEL")
        or existing.get("release_channel")
        or RELEASE_CHANNEL
    ).strip()
    if channel not in _CHANNELS:
        raise BuildMetadataError(f"VESTA_RELEASE_CHANNEL is unsupported: {channel!r}")
