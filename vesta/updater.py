...
or ``git pull`` a copy under ``~/.vesta/source`` and ``pip install -e`` it, so
...
def _default_pip_install(root: Path) -> "subprocess.CompletedProcess[str]":
    from vestahub.proc import console_interpreter, no_window_kwargs
    ...
    # message. See vestahub.proc.console_interpreter.
