...
def local_model_onboarding(project_root: Path) -> dict[str, Any]:
    ...
    derives readiness from ``vestahub.local_onboarding`` so they agree. Never
    downloads a model, starts a service, or reaches a public host on its own.
    """
    from vestahub.local_onboarding import local_onboarding_status
...
def overview(project_root: Path) -> dict[str, Any]:
    ...
    from vestahub.audit import summarize_audit
    from vestahub.ledger import summarize_ledger
...
def cost_firewall(
    ...
    *,
    ...
    from vestahub.audit import GUARD_DENY, POLICY_DENY, read_recent_audit
    from vestahub.budget import budget_status
    from vestahub.policy import list_profiles, resolve_policy
...
def context_waste(project_root: Path) -> dict[str, Any]:
    ...
    from vestahub.context_engine import CLIENT_IGNORE_FILES, profile_context
...
def proof_status(project_root: Path) -> dict[str, Any]:
    ...
    from vestahub.signing import resolve_key
...
def guarded_workflows(project_root: Path) -> dict[str, Any]:
    ...
    from vestahub.guarded import load_templates
...
def set_panic(project_root: Path, on: bool) -> dict[str, Any]:
    from vestahub.budget import budget_status, set_budget

    set_budget(project_root, panic=bool(on))
    from vestahub.audit import record_audit_event
...
def set_policy_profile(project_root: Path, profile: str) -> dict[str, Any]:
    from vestahub.audit import record_audit_event
    from vestahub.policy import set_profile
...
def export_proof(
    ...
    from vestahub.proof import build_proof_bundle, render_proof_markdown
...
def run_benchmark_gate(
    ...
    *,
    ...
    from vestahub.benchmark import benchmark_gate, latest_benchmark_report
...
def available_models(
    ...
    *,
    ...
    from vestahub.paid_api_models import list_paid_api_models
    ...
    from vestahub import provider_balance as _bal
    from vestahub import provider_blocks as _blocks
    from vestahub import provider_reliability as _rel
...
def _ask_direct_api_model(
    ...
    *,
    ...
    through :class:`~vestahub.local_runner.FreeAPIRunner` or its paid subclass
    :class:`~vestahub.local_runner.PaidAPIRunner`, both of which read the API
    ...
    def _spec_for(mid: str) -> dict[str, Any] | None:
        if is_paid:
            from vestahub.paid_api_models import spec_for_model_id as paid_spec

            return paid_spec(mid)
        from vestahub.free_models import spec_for_model_id as free_spec
    ...
    if result.get("status") == "answered_locally":
        ...
        # it actually was. vestahub/lifecycle_schema.json's legacy_mappings
        ...
        # per provider turn (vestahub/local_runner.py). Both writers append to
        ...
        if not result.get("ledger_recorded_per_turn"):
            with contextlib.suppress(Exception):
                from vestahub.cost_model import estimate_tokens
                ...
                # seam the tool-loop path uses (vestahub/local_runner.py).
...
def _note_provider_balance(
    ...
    with contextlib.suppress(Exception):
        ...
        from vestahub import provider_balance
...
def _record_paid_account_cost(
    ...
    *,
    ...
    ledger_error = ""
    try:
        from vestahub.cost_model import estimate_tokens, tier_cost
        from vestahub.ledger import (
            EVENT_MODEL_CALL,
            reconcile_observed_model_calls,
            record_model_call_usage_observed,
        )
        from vestahub.usage_report import (
            UNKNOWN_USAGE,
            ProviderTurnUsage,
            UsageValue,
        )
...
def _settle_paid_account_non_dispatch(
    ...
    *,
    ...
    from vestahub.operation_class import DispatchProof, dispatch_proof

    if dispatch_proof(error_code) is not DispatchProof.NOT_DISPATCHED:
        return _unresolved_paid_account_cost()
    try:
        from vestahub.ledger import record_model_call_not_dispatched
    ...
    *,
    ...
    def _fail(error: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            ...
            return failed
        from vesta.model_registry import default_model, models_for, resolve_id
    ...
    from vestahub.accounts import runner_for_account
    ...
    try:
        ...
        from vestahub.ledger import record_operation_intent
        from vestahub.operation_class import classify_operation
    ...
    try:
        from vestahub.ledger import record_model_call_started
    ...
    if isinstance(result, dict):
        from vestahub.run_state import RunState as _RunState
        from vestahub.run_state import canonical_for_cancel_phase
    ...
    if isinstance(result, dict) and result.get("timed_out"):
        from vesta.provider_contract import normalize_provider_error
        from vestahub.deadlines import is_task_deadline
    ...
    if operation_key:
        with contextlib.suppress(Exception):
            from vestahub.idempotency import complete as complete_operation
    ...
    if tool == "benchmark_run":
        return {
            ...
            "text": (
                ...
                "under .vestahub. No model provider is contacted."
            ),
            "mutates": True,
            "confirm": (
                "Run the local benchmark and write its privacy-safe evidence "
                "under .vestahub? No raw prompts or secrets are stored, and no "
    ...
    if name == "benchmark_run":
        ...
        return {
            ...
            "text": (
                ...
                "Privacy-safe evidence was saved under .vestahub."
            ),
        }
    if name in {"proof_json", "proof_markdown"}:
        fmt = "markdown" if name == "proof_markdown" else "json"
        filename = "proof-bundle.md" if fmt == "markdown" else "proof-bundle.json"
        target = project_root / ".vestahub" / filename
