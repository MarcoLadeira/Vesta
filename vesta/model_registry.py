...
``vestahub/accounts.py`` (CLAUDE_MODELS / CODEX_MODELS / COPILOT_MODELS) with
display names duplicated in ``vesta/provider_contract.py`` (_CLAUDE_DISPLAY …).
...
- **Free API providers** (``kimi``/``gemini``/``groq``/``mistral``) are registered in
  ...
  env key, setup hint, picker labels) lives in ``vestahub/free_models.py``;
...
- **Paid direct-API providers** (``deepseek``) are the same shape as the free
  ...
  per token. Operational spec lives in ``vestahub/paid_api_models.py`` and
  real pricing in ``vestahub/deepseek_pricing.py``; kept out of
...
_REGISTRY: dict[str, tuple[ModelSpec, ...]] = {
    ...
    # ids MUST stay identical to vestahub/free_models.py FREE_MODEL_SPECS
    ...
    # real per-token spend — see vestahub/paid_api_models.py and
    # vestahub/deepseek_pricing.py for the operational spec and cost. V4 Flash
...
def free_providers() -> tuple[str, ...]:
    """Free API providers (``vestahub/free_models.py`` operational specs), in
    registration order."""
    return FREE_PROVIDERS


def paid_direct_providers() -> tuple[str, ...]:
    """Paid direct-API providers (``vestahub/paid_api_models.py`` operational
...
def models_for(provider: str) -> tuple[ModelSpec, ...]:
    ...
    (``~/.vesta/models.json``) so a model a provider ships after this release
    ...
    try:
        from vesta.model_overrides import apply_overrides
...
def catalog() -> dict[str, list[dict[str, str]]]:
    ...
    surface is ``vestahub/free_models.py``.
    """
    return {
        provider: [
