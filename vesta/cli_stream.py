...
``vestahub.gui_pipeline.handle_gui_message`` — the exact pipeline the desktop GUI
...
from vesta.activity import stage_message
from vestahub.completion import verdict_label
from vestahub.boundary_errors import safe_detail

# Activity glyphs (stdout is forced to UTF-8 by vesta.cli.main).
_GLYPH = {
    "pending": "◌",
    "running": "◐",
    "success": "✓",
...
def _terminal_verdict(result: dict[str, Any]) -> tuple[str, str, str, str] | None:
    ...
    if "run_result" in result:
        from vestahub.run_result import terminal_presentation
...
def stream_ask(
    ...
    *,
    ...
    """
    from vesta.brand import cli_header
    ...
    if json_out:
        ...
        if (terminal := _terminal_verdict(result)) is not None:
            ...
            if verdict != "completed":
                from vestahub.recovery_actions import render_recovery_text
