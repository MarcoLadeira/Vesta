...
from vesta.gui_lifecycle import drain_workers
...
from vesta.gui_nav import DEFAULT_VIEW, find_nav, nav_groups
...
from vesta.gui_view_model import SECTIONS, build_view_model
...
from vesta.message_render import render_message_html

INSTALL_HINT = (
    'Install desktop GUI support with: python -m pip install "vesta[desktop-gui]" '
...
def run_once(project_root: Path) -> dict[str, Any]:
    ...
    from vestahub.gui_pipeline import last_savings_receipt
...
# sans, SIL OFL) ships in vesta/assets/fonts and is loaded at startup, so the app
...
from vestahub.autonomy import MODE_LABELS  # noqa: E402
