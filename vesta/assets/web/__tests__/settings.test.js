...
describe("section registry (#236)", () => {
  it("exposes the target taxonomy in order", () => {
    ...
    expect(VestaSettings.sections.map((s) => s.group)).toEqual([
      "Vesta",
      "Vesta",
      "AI",
      "AI",
  ...
  it("every section has a unique id, title, keywords, and a render function", () => {
    expect(new Set(VestaSettings.sections.map((section) => section.id)).size).toBe(
      VestaSettings.sections.length,
    );
  ...
  it("keeps historical Settings links mapped to their canonical destination", () => {
    expect(VestaSettings.resolveSettingsTarget("overview")).toEqual({
      sectionId: "general",
      subsectionId: "defaults",
    });
    expect(VestaSettings.resolveSettingsTarget("providers")).toEqual({
      sectionId: "connections",
      subsectionId: "connections",
    });
    expect(VestaSettings.resolveSettingsTarget("balance")).toEqual({
      sectionId: "usage",
      subsectionId: "balances",
    });
    expect(VestaSettings.resolveSettingsTarget("firewall")).toEqual({
      sectionId: "usage",
      subsectionId: "budgets",
    });
    expect(VestaSettings.resolveSettingsTarget("permissions")).toEqual({
      sectionId: "safety",
      subsectionId: "permissions",
    });
    expect(VestaSettings.resolveSettingsTarget("privacy")).toEqual({
      sectionId: "safety",
      subsectionId: "privacy",
    });
    expect(VestaSettings.resolveSettingsTarget("tools")).toEqual({
      sectionId: "advanced",
      subsectionId: "tools",
    });
    expect(VestaSettings.resolveSettingsTarget("about")).toEqual({
      sectionId: "advanced",
      subsectionId: "about",
    });
  });
  ...
  it("indexes old product language without indexing credential values", () => {
    const terms = Object.fromEntries(
      VestaSettings.sections.map((section) => [section.id, section.keywords]),
    );
...
describe("Appearance theme picker", () => {
  ...
  it("falls back to the theme the window is wearing when the payload has none", () => {
    globalThis.VestaTheme = { current: () => ({ preference: "system", theme: "light" }) };
...
describe("About page: update status (mandatory-update system)", () => {
  const esc = (s) =>
    String(s == null ? "" : s)
      ...
      .replace(/"/g, "&quot;");
  const ctx = { esc, state: { boot: {} } };
  const section = () => VestaSettings.sections.find((s) => s.id === "advanced");
  const base = {
    version: "0.2.1a1",
