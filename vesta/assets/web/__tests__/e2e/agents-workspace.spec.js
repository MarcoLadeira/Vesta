...
test('team limits are captured exactly and keyboard editing cannot change permission mode', async ({ page }, testInfo) => {
  ...
  const mode = await page.evaluate(() => window.__vesta.state.mode.id);
  ...
  expect(await page.evaluate(() => window.__vesta.state.mode.id)).toBe(mode);
  ...
  await page.evaluate(() => {
    window.__vesta.setAgentsRunSettings({ maxParallel: 4, budgetUsd: '9' });
    window.__vesta.send(window.__vesta.state.lastSend);
  });
  expect(await page.evaluate(() => window.__mock.lastRequest)).toMatchObject({ maxParallel: 3, budgetUsd: '1.234000000000000001' });
  await page.evaluate(() => window.__vesta.applyBootSelection(window.__vesta.state.boot));
  expect(await page.evaluate(() => [window.__vesta.state.agentsMaxParallel, window.__vesta.state.agentsBudgetUsd])).toEqual([2, '']);
});
...
test('invalid pre-send budget keeps the draft and does not dispatch', async ({ page }) => {
  ...
  await page.evaluate(() => window.__vesta.setAgentsRunSettings({ budgetUsd: '-1' }));
...
test('live objective updates continue after acknowledgement and reject older revisions', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__vesta.state.busy)).toBe(false);
  expectNoFatalErrors(diagnostics);
});

test('objective selection preserves canonical worktree and receipt targets', async ({ page }) => {
  const recorded = { ...objective, receipt: { report: 'vesta-objective-receipt', objective_id: 'obj-1' }, assignments: [{ ...objective.assignments[0], worktree: '/workers/a', branch: 'codex/a', receipt: { report: 'vesta-agent-receipt', assignment_id: 'a-1' } }] };
...
test('multiple agents checkbox preserves permission mode and model, persists, and travels with retry', async ({ page }) => {
  const diagnostics = await openApp(page, { boot: { prefs: { multiAgentEnabled: true } } });
  const selection = await page.evaluate(() => ({ mode: window.__vesta.state.mode.id, model: window.__vesta.state.model.id }));
  await page.locator('#moreBtn').click();
  await page.getByRole('menuitem', { name: /Change permissions/ }).click();
  ...
  await page.evaluate(() => window.__vesta.setMultiAgentEnabled(false));
  await page.evaluate(() => window.__vesta.send(window.__vesta.state.lastSend));
...
test('cloud permission is explicit and applies only to the next objective', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__vesta.state.agentsAllowCloud)).toBe(false);
  ...
  await page.evaluate(() => {
    window.__vesta.state.model.kind = 'free';
    window.__vesta.state.freeConsent.add(window.__vesta.state.model.id);
  });
...
test('objective signal releases chat only for the active request and workspace', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__vesta.state.busy)).toBe(true);
...
test('older objective and dashboard responses cannot replace a newer request or another view', async ({ page }) => {
  ...
  expect(await page.evaluate(() => ({ view: window.__vesta.state.view, requestId: window.__vesta.state.currentRequest, busy: window.__vesta.state.busy }))).toEqual({ view: 'chat', requestId: newId, busy: true });
