...
test("a run past the cap shows an honest truncation marker, ledger stays whole", async ({ page }) => {
  await page.evaluate(() => { window.__VESTA_EVENT_CAP__ = 20; });
  ...
  const { kept, truncated } = await page.evaluate(() => ({
    kept: window.__vesta.state.store.list().length,
    truncated: window.__vesta.state.store.truncatedCount(),
  }));
...
test("a completed capped turn keeps its honest truncation marker", async ({ page }) => {
  ...
  const truncated = await page.evaluate(() => window.__vesta.state.store.truncatedCount());
