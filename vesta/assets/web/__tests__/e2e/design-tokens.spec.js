...
test("chrome uses accessible SVG icons rather than emoji glyphs", async ({ page }) => {
  await openApp(page);
  // #newApp then #newChat carried this assertion as each button was removed;
  // #headerNewChat is the same shape and is the one now on screen.
  await expect(page.locator("#headerNewChat .ui-icon")).toBeVisible();
  await expect(page.locator("#buildToggle .ui-icon")).toBeAttached();
  await expect(page.locator("#headerNewChat")).not.toContainText("✦");
  await expect(page.locator("#buildToggle")).not.toContainText("✦");
  await expect(page.locator("#headerNewChat .ui-icon")).toHaveAttribute(
    "aria-hidden",
    "true"
  );
});
...
test("rendered design-token preview documents the canonical scales", async ({ page }) => {
  await page.goto("/vesta/assets/web/design-tokens-preview.html");
