...
test("the global model picker manages hidden and custom models without showing hidden choices", async ({ page }) => {
  ...
  await expect(page.getByText("Global · ~/.vesta/models.json")).toBeVisible();
...
test("the run-mode select offers every mode, in order, Auto-apply included", async ({ page }) => {
  ...
  await expect(page.locator("#settingsPage")).toContainText("what Vesta starts in, every time", seen);
});
...
test("an invalid usage limit shows an inline error and saves nothing", async ({ page }) => {
  ...
  await openApp(page, {
    settings: {
      ...
      usage: [{ modelId, provider: "claude", source: "vesta", metric: "tokens", used: 100, limit: null, remaining: null, percent: 0, window: "month", confidence: "measured" }],
    },
  });
  await openNav(page, "Settings");
  await railItem(page, "firewall").click();
...
test("a large token total is made legible by the model-call count (#334)", async ({ page }) => {
  ...
  await openApp(page, {
    settings: {
      ...
      usage: [{ modelId, provider: "gemini", source: "vesta", metric: "tokens", used: 700000, limit: null, window: "month", confidence: "measured", modelCalls: 87, taskCount: 5 }],
    },
  });
  await openNav(page, "Settings");
  await railItem(page, "firewall").click();
