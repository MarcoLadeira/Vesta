...
test("Stop is acknowledged immediately without claiming the work stopped", async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__vesta.state.message.status)).toBe("cancel_requested");
});
...
test("confirmed teardown is what turns the run cancelled", async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__vesta.state.message.status)).toBe("cancelled");
