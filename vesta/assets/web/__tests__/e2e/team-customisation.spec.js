...
test('timestamps are centered and map keeps the composer with real directed links', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('dialog')).toContainText('Choose Configure');
  ...
  await page.getByLabel('Agent options', { exact: true }).click();
...
test('the global avatar strip opens agents on every page without navigating away', async ({ page }, testInfo) => {
  ...
  for (const [command, view] of [['settings', 'settings'], ['prompts', 'prompts'], ['savings', 'home']]) {
    await page.evaluate((id) => window.__vesta.runCommand(id), command);
    ...
    await avatar.click();
    expect(await page.evaluate(() => window.__vesta.state.view)).toBe(view);
  ...
  await page.evaluate(() => window.__vesta.runCommand('settings'));
  ...
  expect(await page.evaluate(() => window.__vesta.state.view)).toBe('agents');
...
test('targeted composer fences messages and keeps recipient drafts across acknowledgements', async ({ page }) => {
  await page.setViewportSize({ width: 1500, height: 980 });
  const diagnostics = await start(page);
  await page.locator('#input').fill('Team draft');
  await page.getByRole('button', { name: 'Organise team', exact: true }).click();
  await page.locator('[data-map-select="alex"]').click();
  await expect(page.locator('#teamRecipient')).toHaveValue('alex');
  await expect(page.locator('#input')).toHaveValue('');
  await page.locator('#input').fill('/panic is just text for this agent');
  await page.locator('#input').press('Enter');
  await page.locator('#input').press('Enter');
  expect(await page.evaluate(() => window.__mock.objectiveControls.length)).toBe(1);
  const first = await page.evaluate(() => window.__mock.objectiveControls.at(-1));
  expect(first).toEqual({ objective_id: 'custom-team', assignment_id: 'alex', action: 'agent_message', value: { revision: 0, message: '/panic is just text for this agent' } });
  await page.locator('#teamRecipient').selectOption('taylor');
  await page.locator('#input').fill('Taylor draft');
  await page.evaluate((request) => window.__mock.emitObjectiveControl({ ok: false, workspaceRoot: '/demo', error: 'Retry this message', control: { ...request, revision: request.value.revision } }), first);
  await expect(page.locator('#input')).toHaveValue('Taylor draft');
  await page.locator('#teamRecipient').selectOption('alex');
  await expect(page.locator('#input')).toHaveValue('/panic is just text for this agent');
  await page.locator('#input').press('Enter');
  expect(await page.evaluate(() => window.__mock.objectiveControls.length)).toBe(2);
  await page.locator('#input').fill('New draft while the message is being queued');
  await page.evaluate(({ request, objective }) => window.__mock.emitObjectiveControl({ ok: true, workspaceRoot: '/demo', objective: { ...objective, revision: 6, team_revision: 1 }, control: { ...request, revision: request.value.revision } }), { request: first, objective });
  await expect(page.locator('#input')).toHaveValue('New draft while the message is being queued');
  await page.locator('#teamRecipient').selectOption('taylor');
  await expect(page.locator('#input')).toHaveValue('Taylor draft');
  await page.locator('#teamRecipient').selectOption('');
