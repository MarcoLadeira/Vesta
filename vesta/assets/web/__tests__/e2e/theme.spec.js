...
test("a theme that lands without the cross-fade lands everywhere at once", async ({ page }) => {
  ...
  for (const next of ["dark", "vesta", "light", "viber-coder"]) {
    const started = await page.evaluate((palette) => {
      window.__vesta.applyAppearance({ theme: palette });
...
test("every theme's stars have their own matching starlight", async ({ page }) => {
  ...
  const starlight = {};
  for (const palette of PALETTES) {
    await page.evaluate((next) => window.__vesta.applyAppearance({ theme: next }), palette);
...
function richAnswer() {
  return {
    ...
    changed_files: [" M vesta/assets/web/app.js", " A vesta/assets/web/theme.js"],
    presentation: {
      schema_version: 1,
    ...
    workflow: {
      ...
      diff_review: {
        ...
        files: [
          {
            path: "vesta/assets/web/app.js", decision: "approved", additions: 3, deletions: 1,
          ...
          { path: "vesta/assets/web/theme.js", decision: "approved", additions: 1, deletions: 0, hunks: [] },
        ],
      },
    },
  };
