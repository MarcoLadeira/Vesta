...
const esc = (s) =>
  String(s == null ? "" : s)
    ...
    .replace(/"/g, "&quot;").replace(/'/g, "&#39;");
const uiIcon = (name, options) => window.VestaIcons.icon(name, options);
...
if (typeof window !== "undefined") {
  window.VestaModePresentationLabel = modePresentationLabel;
  window.VestaModePresentationCopy = modePresentationCopy;
}
...
function mdToHtml(src) {
  return window.VestaMarkdown.render(src);
}

function userMessageHtml(text) {
  return window.VestaChatComponents.renderUserMessage(text);
}

function responseProseHtml(markdownHtml) {
  return window.VestaChatComponents.renderProseRegion(markdownHtml);
}

function responseShellHtml(headerHtml, contentHtml) {
  return window.VestaChatComponents.renderResponseShell({
...
function assistantPresentationHtml(headerHtml, text, presentation, options = {}) {
  return window.VestaChatComponents.renderAssistantPresentation({
    density: state.responseDensity,
    headerHtml,
...
function renderStreamingBody(body, text, addedCharacters = 0) {
  ...
  body.innerHTML = window.VestaMarkdown.render(text, { streaming: true });
...
function applyAppearance(prefs) {
  ...
  if (window.VestaTheme) window.VestaTheme.apply(p.theme);
  ...
  const responseDensity = window.VestaChatComponents.normalizeResponseDensity(requestedResponseDensity);
...
window.VestaModelVisibility = isModelVisible;
...
function boot() {
  bridge.boot((json) => {
    ...
    if (window.VestaComposer) window.VestaComposer.applyBootStyle();
    switchView("chat");
    ...
    if (window.VestaOnboarding && !(b.resume && b.resume.requires_choice)) {
      window.VestaOnboarding.maybeStart(onboardingCtx());
    }
  ...
  if (window.VestaMessageState && bridge.reportIllegalTransition) {
    window.VestaMessageState.setRefusalSink((from, to) => bridge.reportIllegalTransition(from, to));
  }
...
// One brand voice, one source: copy comes from vesta/brand.py via the boot
...
function renderUpdateBanner(update) {
  ...
  const states = {
    ...
    policy_blocked: [policy.owner && policy.owner !== "vesta" ? "Managed by administrator" : "Updates disabled by policy", "Vesta will not race another update owner.", "neutral"],
  ...
  try { window.dispatchEvent(new CustomEvent("vesta-update-state", { detail: update })); } catch (_e) { /* old web engine */ }
  if (!visible) {
...
function mountStarfield() {
  if (starfield || !global0().VestaStarfield) return;
  starfield = global0().VestaStarfield.mount(document.getElementById("starfall"));
}

// The sky is drawn on a canvas, not with CSS, so a theme change has to tell it
// to pick up the new starlight token.
window.addEventListener("vesta:themechange", () => {
...
function updateComposerAvailability() {
  ...
  if (window.VestaComposer) window.VestaComposer.refresh();
...
function updateCliMirror() {
  ...
  el.textContent = VestaActivity.cliMirror(state.model.id, state.mode.id, task);
}
...
function updateInspectorLive(stepText) {
  ...
  $("#inspLiveElapsed").textContent = VestaActivity.formatElapsed(Date.now() - state.startTime);
...
function sendBuild(value) {
  ...
  state.message = VestaMessageState.beginRequest(requestId, {});
  state.message = VestaMessageState.transition(state.message, "preparing");
  state.store = VestaActivity.createStore();
...
function onBuildReply(json) {
  ...
  if (!VestaMessageState.canApply(state.message, d.requestId)) return; // stale reply ignored
  const r = d.result || {};
  const backendStatus = r.status || "failed";
  state.message = VestaMessageState.transition(
    state.message,
    VestaMessageState.fromBackendStatus(backendStatus, r.completion_verdict, r.run_result),
  );
...
function appendMsg(html, cls, timestamp) {
  ...
  withChatScrollPreserved(() => {
    const thread = $("#thread");
    const stamp = cls === "user" && window.VestaChatTime?.stamp(timestamp === undefined ? new Date() : timestamp);
...
function roleHeader(label, color, opts) {
  return window.VestaChatComponents.renderAssistantHeader({
    label,
    color,
    copy: !!(opts && opts.copy),
...
function send(retryOf) {
  ...
  if (sel.multiAgentEnabled) {
    ...
    try {
      Object.assign(sel, window.VestaAgentsWorkspace.runSettings(sel.maxParallel ?? 2, sel.budgetUsd ?? null));
    } catch (error) {
      toast(error.message);
      return;
    }
  ...
  const requestId = (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : "r" + Date.now() + Math.random();
  state.currentRequest = requestId;
  state.message = VestaMessageState.beginRequest(requestId, { retryOf: retryOf ? state.lastFailedRequestId : null });
  state.message = VestaMessageState.transition(state.message, "preparing");
  state.store = VestaActivity.createStore();
...
function buildPending(sel) {
  ...
  const initialStage = VestaActivity.stageMessage(0, { modelLabel: sel.modelLabel || sel.model }).stage;
...
function renderTimeline() {
  ...
  const grouped = VestaActivity.groupRows(state.store.list());
...
function stripElapsed(ms) {
  const strip = $("#statusStrip");
  if (!strip.hasAttribute("hidden")) $("#ssTime").textContent = VestaActivity.formatElapsed(ms);
}
...
function stripFinalize(status, r) {
  const strip = $("#statusStrip");
  if (strip.hasAttribute("hidden")) return;
  $("#ssTime").textContent = VestaActivity.formatElapsed(state.startTime ? Date.now() - state.startTime : 0);
...
function applyActivityState(event, deferLiveUpdate = false) {
  ...
  if (next) state.message = VestaMessageState.transition(state.message, next);
...
function onActivity(json) {
  const d = JSON.parse(json);
  if (!VestaMessageState.canApply(state.message, d.requestId)) return; // stale guard
...
function onToken(json) {
  const d = JSON.parse(json);
  if (!VestaMessageState.canApply(state.message, d.requestId)) return; // stale guard
  state.message = VestaMessageState.transition(state.message, "streaming");
...
function updateGenStage(sel) {
  ...
  const sm = VestaActivity.stageMessage(elapsedMs / 1000, { streaming: state.streaming, modelLabel: sel.modelLabel || sel.model });
  ...
  if (tEl) tEl.textContent = VestaActivity.formatElapsed(elapsedMs);
...
function stop() {
  ...
  state.message = VestaMessageState.transition(state.message, "cancel_requested");
  stopTimer();
...
function finishCancel(cancelledId, teardown) {
  ...
  state.message = VestaMessageState.transition(state.message, "cancelled");
...
function openModelPicker() {
  ...
  setTimeout(() => {
    if (window.VestaComposer && typeof window.VestaComposer.openModel === "function") {
      window.VestaComposer.openModel();
      return;
    }
...
function completionVerdict(r) {
  return VestaRunResult.fromResult(r);
}
// The one user-facing label per verdict — mirrors vestahub.completion.VERDICT_LABELS
...
function metaFooter(r, sel, durMs) {
  ...
  const elapsed = VestaActivity.formatElapsed(durMs);
...
// (vestahub/accounts.py, connection_for_account).
//
...
function finalize(status, r) {
  ...
  el.innerHTML = assistantPresentationHtml(
    ...
    r && r.presentation,
    {
      ...
      elapsed: VestaActivity.formatElapsed(durMs),
...
// rows mark where the backend truncated a hunk (vestahub/diff_review.py).
...
// Decisions here only record human sign-off (vestahub/diff_review.py never
...
function completeReply(d) {
  ...
  state.message = VestaMessageState.transition(
    state.message,
    VestaMessageState.fromBackendStatus(
      backendStatus,
...
function renderDashboard(section, quiet = false) {
  ...
  const paint = (json) => {
    ...
    if (section === "agents" && window.VestaAgentsWorkspace) {
...
function paintAgentsWorkspace() {
  ...
  window.VestaAgentsWorkspace.mount($("#dashPage"), state.agentsSnapshot, {
...
function paintAgentChatCards() {
  if (!window.VestaAgentsWorkspace) return;
  ...
  document.querySelectorAll("[data-agent-chat-objective]").forEach((element) => {
    ...
    if (!objective) return;
    const html = window.VestaAgentsWorkspace.renderCompact(objective) + (window.VestaAgentsTeam ? window.VestaAgentsTeam.feedHtml(objective) : '');
...
function syncTeamRecipient(objective) {
  ...
  const people = window.VestaAgentsTeam.agents(objective);
  const html = '<option value="">To: Team</option>' + people.map((a, index) => '<option value="' + esc(a.assignment_id) + '">To: ' + esc(window.VestaAgentsTeam.name(a, index)) + '</option>').join('') + (target && agent && !people.some((a) => a.assignment_id === agent.assignment_id) ? '<option value="' + esc(agent.assignment_id) + '">To: ' + esc(window.VestaAgentsTeam.name(agent, 0)) + '</option>' : '') + (target && !agent ? '<option value="' + esc(state.teamAgentId) + '">Agent unavailable</option>' : '');
  ...
  input.placeholder = target ? 'Ask ' + (agent ? window.VestaAgentsTeam.name(agent, 0) : 'this agent') + ' · queued after the current task…' : enabled ? 'Give your team a new objective…' : teamComposer.placeholder;
  updateComposerAvailability();
}
function sendAgentMessage(target) {
  ...
  teamComposer.pending = { ...request, key: teamComposer.key, name: window.VestaAgentsTeam.name(target.agent, 0), root: state.boot.workspace?.root };
  teamComposer.drafts.set(teamComposer.key, $('#input').value);
...
function teamMapOptions(objective) {
  ...
  return {
    ...
    onAdd: () => window.VestaTeamMap.addDialog(objective, teamMapOptions(objective)),
...
function paintAgentTeam() {
  ...
  if (!element || !window.VestaAgentsTeam) return;
  ...
  if (map) {
    ...
    if (!map.hidden && !document.querySelector('.team-edit-dialog[open]')) window.VestaTeamMap.mount(map, objective, teamMapOptions(objective));
  }
  const strip = $('#agentsTeamStrip');
  if (strip && !strip.hidden) {
    const html = window.VestaAgentsTeam.stripHtml(objective, objectives, state.teamOpen ? state.teamAgentId : null);
  ...
  window.VestaAgentsTeam.mountPanel(element, objective, state.teamAgentId, {
...
function settingsCtx(d) {
  return {
    ...
    replayTour: () => { if (window.VestaOnboarding) window.VestaOnboarding.replay(onboardingCtx()); },
    startDoctorRefresh() {
...
function renderSettings() {
  ...
  const paint = (json) => {
    ...
    window.VestaSettings.render(page, settingsCtx(d));
  };
...
const APPROVAL_SCOPE = {
  ...
  benchmark_run: { risk: "Local evidence write", scope: ".vestahub benchmark history (privacy-safe metadata; no raw prompts)" },
  proof_json: { risk: "Local file write", scope: ".vestahub/proof-bundle.json (redacted and locally signed)" },
  proof_markdown: { risk: "Local file write", scope: ".vestahub/proof-bundle.md (redacted and locally signed)" },
};
...
function teamPaletteItems() {
  ...
  if (target?.agent) {
    const name = window.VestaAgentsTeam.name(target.agent, 0);
