(function (global) {
  ...
  function feedHtml(objective, selectedId) {
    ...
    const timeline = events.map((event) => {
      ...
      const stamp = global.VestaChatTime?.stamp(event.occurred_at);
  ...
  function conversationHtml(objective, selected) {
    ...
    const time = (value) => { const stamp = global.VestaChatTime?.stamp(value); return stamp ? '<time class="chat-timestamp" datetime="' + esc(stamp.iso) + '">' + esc(stamp.label) + '</time>' : ''; };
  ...
  function compactInspector(element, objective, agent) {
    ...
    if (!agent.pending_approval && agent.status === 'running') {
      ...
      if (report?.timestamp) { const stamp = global.VestaChatTime?.stamp(report.timestamp); if (stamp) current.innerHTML += '<time datetime="' + esc(new Date(report.timestamp).toISOString()) + '">Reported ' + esc(stamp.label) + '</time>'; }
  ...
  global.VestaAgentsTeam = { feedHtml, panelHtml, mountPanel, stripHtml, name, avatar, agents, actorId, modelOptions, state, settle, conversationHtml, activities, currentActivity, attentionHtml, setupState };
