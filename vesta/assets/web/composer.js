 ...
 * (docs/design/vesta-prompt-composer-redesign.md) as one switchable card:
 *
...
(function (global) {
  ...
  function state() { return (global.__vesta && global.__vesta.state) || {}; }
  function boot() { return state().boot || {}; }
  function icon(name) { return global.VestaIcons ? global.VestaIcons.icon(name) : ""; }
  function esc(s) {
  ...
  function modeLabelOf(mode) {
    if (typeof global.VestaModePresentationLabel === "function") {
      return global.VestaModePresentationLabel(mode);
    }
  ...
  // vesta-model-picker-redesign.md §14 "State ownership"). Selection itself
  ...
  function pickerModels() {
    return (boot().models || []).filter(function (m) {
      return m && m.kind !== "auto" && m.group !== "routing" &&
        (!global.VestaModelVisibility || global.VestaModelVisibility(m));
    });
  }
  ...
  function buildModelPop() {
    ...
    if (manage) manage.onclick = function () {
      closePopovers();
      if (global.__vesta && global.__vesta.openSettings) global.__vesta.openSettings("models");
    };
  }
  ...
  function buildMorePop() {
    ...
    var map = {
      ...
      shortcuts: function () { closePopovers(); if (global.__vesta && global.__vesta.runCommand) global.__vesta.runCommand("shortcuts"); },
    };
  ...
  function init() {
    ...
    if (teamButton) teamButton.onclick = function () {
      if (!global.__vesta) return;
      if (state().multiAgentEnabled) openTeam();
      else global.__vesta.setMultiAgentEnabled(true);
    };
  ...
  // VestaComposer.refresh()/applyStyle() at the right lifecycle points.
