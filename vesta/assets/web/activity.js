/* Pure activity/cancellation logic shared by the browser UI and the Vitest
   suite. Framework-free UMD-lite: attaches to window.VestaActivity in the
   browser and exports for Node/Vitest. Mirrors vesta/activity.py so the two
...
(function (global) {
  ...
  function createStore(cap) {
    var CAP = cap
      || (typeof window !== "undefined" && +window.__VESTA_EVENT_CAP__)
      || DEFAULT_CAP;
    var events = [];
  ...
  // vesta/brand.py::cli_mirror so both surfaces render the same command.
