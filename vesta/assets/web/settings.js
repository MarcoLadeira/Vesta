 ...
 *
 * UMD: a plain <script> in the browser (sets window.VestaSettings) and a CommonJS
...
(function (global) {
  ...
  // MIRROR of vestahub/autonomy.py MODE_LABELS — the single source of truth for
  ...
  function modePresentationLabel(id, fallback) {
    ...
    if (typeof global.VestaModePresentationLabel === "function") {
      return global.VestaModePresentationLabel({ id: id, label: raw });
    }
    return raw;
  }
  function modePresentationCopy(value) {
    if (typeof global.VestaModePresentationCopy === "function") {
      return global.VestaModePresentationCopy(value);
    }
    return String(value || "")
  ...
  function modelsHtml(d, ctx) {
    ...
    var modelOverrides = d.modelOverrides || { global: true, path: "~/.vesta/models.json", providers: {}, hidden: {}, errors: [] };
    ...
    h += '<div class="set-note">Global · ' + esc(modelOverrides.path || "~/.vesta/models.json") + '. Show or hide models everywhere. Availability stays separate: unavailable models keep their reason.</div>';
  ...
  function appearanceHtml(d, ctx) {
    ...
    var appliedTheme = global.VestaTheme ? global.VestaTheme.current().preference : null;
    h +=
  ...
  function wire(page, ctx) {
    ...
    wireUpdateButtons();
    if (global.__vestaSettingsUpdateListener) {
      global.removeEventListener("vesta-update-state", global.__vestaSettingsUpdateListener);
    }
    global.__vestaSettingsUpdateListener = function (event) {
    ...
    global.addEventListener("vesta-update-state", global.__vestaSettingsUpdateListener);
      ...
      segment.querySelectorAll("button").forEach(function (button) {
        button.onclick = function () {
          ...
          bridge.setUpdatePolicy(key, on ? "true" : "false", function (json2) {
            ...
            if (result.ok && global.__vesta && global.__vesta.renderUpdateBanner) {
              global.__vesta.renderUpdateBanner(result);
            }
    ...
    page.querySelectorAll("[data-composer-style-key]").forEach(function (segment) {
      ...
      segment.querySelectorAll("button").forEach(function (button) {
        button.onclick = function () {
          ...
          if (global.VestaComposer) global.VestaComposer.setStyle(button.dataset.value);
        };
      });
    });
