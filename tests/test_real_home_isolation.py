...
developer's real ``~/.claude/CLAUDE.md``, ``~/.opai/{status.txt,global.json,
instructions,integrations,bin,journal}`` and ``~/.agents/skills/opai``. The
...
from opai import installer, integrations
...
HOME_RESOLVING = {
    ...
    "ensure_opai_skill_library": integrations.ensure_opai_skill_library,
    "uninstall_opai": integrations.uninstall_opai,
    "update_opai_source": integrations.update_opai_source,
...
def _subprocess_subcommand(node: ast.AST) -> str | None:
    """The activating subcommand a ``[python, "-m", "opai", ...]`` argv runs."""
    items = _string_items(node)
    for index in range(len(items) - 2):
        if (
            items[index] == "-m"
            and items[index + 1] == "opai"
            ...
            if subcommand and not module.isolated(node):
                found.append(
                    (
                        path.name,
                        line,
                        f"`-m opai {subcommand}` subprocess outside isolated_home()",
                    )
                )
...
class VisibilityTests(unittest.TestCase):
    def test_activate_repair(self):
        ...
        subprocess.run([sys.executable, "-m", "opai", "activate"])
    ...
    def test_dry_run_and_explicit_homes_are_fine(self):
        ...
        integrations.ensure_opai_skill_library(root, home)
...
"""
        ...
        self.assertEqual(
            ...
            [
                ...
                "`-m opai activate` subprocess outside isolated_home()",
            ],
        )
