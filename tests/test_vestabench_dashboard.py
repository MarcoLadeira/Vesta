"""VestaBench runner + dashboard (#180): deterministic, local, regression-aware."""
...
from vestahub import vestabench

from vestahub.vestabench import (
    DIMENSIONS,
    SCENARIOS,
    build_vestabench_dashboard,
    detect_regressions,
    latest_vestabench_report,
    read_vestabench_history,
    run_vestabench,
)
...
class RunnerTests(unittest.TestCase):
    ...
    def test_crashing_scenarios_fail_instead_of_crashing_the_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            report = run_vestabench(Path(tmp), write=False, scenarios=(scenario,))
    ...
    def test_runs_append_to_durable_history(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            first = run_vestabench(root)
            second = run_vestabench(root)
            history = read_vestabench_history(root)
    ...
    def test_latest_report_reads_the_most_recent_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.assertIsNone(latest_vestabench_report(root))
            run_vestabench(root)
            newest = run_vestabench(root)
            self.assertEqual(latest_vestabench_report(root)["run_id"], newest["run_id"])

    def test_run_reads_only_the_previous_history_record(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with mock.patch.object(
                vestabench, "read_vestabench_history", return_value=[]
            ) as history:
                run_vestabench(root, write=False, scenarios=())
...
class DashboardTests(unittest.TestCase):
    def test_dashboard_renders_dimensions_cost_and_history(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_vestabench(root)
            run_vestabench(root)
            path = build_vestabench_dashboard(root)
    ...
    def test_dashboard_without_runs_points_to_the_run_command(self):
        with tempfile.TemporaryDirectory() as tmp:
            html = build_vestabench_dashboard(Path(tmp)).read_text(encoding="utf-8")
        self.assertIn("No VestaBench run recorded yet", html)
        self.assertIn("vesta hub vestabench run", html)
...
class CliTests(unittest.TestCase):
    ...
    def test_cli_run_reports_and_exits_zero_when_all_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            code, output = self._main(["--project", tmp, "vestabench", "run"])
            self.assertEqual(code, 0)
            payload = json.loads(output)
            self.assertEqual(payload["kind"], "vestabench")

            code, output = self._main(["--project", tmp, "vestabench", "dashboard"])
            self.assertEqual(code, 0)
            self.assertTrue(Path(json.loads(output)["path"]).exists())

            code, output = self._main(
                ["--project", tmp, "vestabench", "history", "--limit", "5"]
            )
            self.assertEqual(code, 0)
