...
from opai.integrations import render_statusline
from opai.terminal_ui import (
    BLUE,
    RESET,
    SOFT_BLUE,
...
class OPaiTerminalUITests(unittest.TestCase):
    def test_statusline_is_blue_and_right_aligned(self):
        self.assertEqual(
            render_statusline(width=24), f"              {BLUE}Using OPai{RESET}"
        )

    def test_statusline_can_be_plain_for_machine_consumers(self):
        self.assertEqual(
            render_statusline(width=24, color=False), "              Using OPai"
        )
    ...
    def test_welcome_compact_includes_badge_and_graphic(self):
        ...
        self.assertIn(f"{BLUE}Using OPai{RESET}", welcome)
        self.assertIn("OPai", welcome)
    ...
    def test_animation_frames_move_the_model(self):
        ...
        self.assertIn(f"{BLUE}Using OPai{RESET}", frames[0])
    ...
    def test_play_animation_uses_terminal_controls(self):
        ...
        self.assertIn("Using OPai", output)
